...
result_t ds1337_get_time (I2C_BUS i2c_bus_id, u8 adr_chip, time_t *t)
{
    ...
    if (i2c_read_reg(i2c_bus_id, adr_chip, DS1337_REG_SEC, &tmp) != SUCCESS)
        return ERROR;
    bcd_2_bin(tmp, &t->sec);

    if (i2c_read_reg(i2c_bus_id, adr_chip, DS1337_REG_MIN, &tmp) != SUCCESS)
        return ERROR;
    bcd_2_bin(tmp, &t->min);

    if (i2c_read_reg(i2c_bus_id, adr_chip, DS1337_REG_HOURS, &tmp) != SUCCESS)
        return ERROR;
    bcd_2_bin(tmp, &t->hrs);

    if (i2c_read_reg(i2c_bus_id, adr_chip, DS1337_REG_DOW, &tmp) != SUCCESS)
        return ERROR;
    bcd_2_bin(tmp, &t->dow);

    if (i2c_read_reg(i2c_bus_id, adr_chip, DS1337_REG_DAY, &tmp) != SUCCESS)
        return ERROR;
    bcd_2_bin(tmp, &t->day);

    if (i2c_read_reg(i2c_bus_id, adr_chip, DS1337_REG_MONTH, &tmp) != SUCCESS)
        return ERROR;
    bcd_2_bin(tmp, &t->mth);

    if (i2c_read_reg(i2c_bus_id, adr_chip, DS1337_REG_YEAR, &tmp) != SUCCESS)
        return ERROR;
    bcd_2_bin(tmp, &t->yrs);
