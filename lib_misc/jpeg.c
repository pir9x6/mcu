...
void nameinit(char* DirName)
{
    ...
    f_mount(0, &fs);
    if (f_opendir(&dirs,DirName) == FR_OK)
     {
...
/*    while(*path!= '.')
    ...
    path++;
    while(*path)
    {   //ExtensionName[4]  j  没定义
        if(*path>='a' && *path<='z')ExtensionName[j]=(*p++)-0x20;
        else ExtensionName[j]=*p++;
        j++;
    }*/        
    //已经调用了nameinit();
     if((finfo.fname[9]=='B')&&(finfo.fname[10]=='M')&&(finfo.fname[11]=='P')) //得到BMP图片,改变显示屏的显示方向
    {    

    /*
     LCD_WR_REG(0x0003,(1<<12)|(1<<5)|(1<<4)|(1<<3) );     //扫描方向控制 左至右,下至上                 
        //设置面板大小
    ...
    Lcd_WR_Start();
    Set_Rs;        */
//        Lcd_SetCursor(0x00, 0x0000);    //设置X坐标点  设置Y坐标点               
...
{    
    ...
    //算出显示图象的长，宽
    //显示区域无效    
//    if((PICINFO.S_Height==0||PICINFO.S_Width==0)||((PICINFO.S_Height!==PICINFO.ImgHeight)||(PICINFO.S_Width!==PICINFO.ImgHeight)))
//    {
//        return FALSE;   
//    } 
    f_mount(0, &fs);
    if (f_opendir(&dirs,FileName) == FR_OK)
     {
            while (f_readdir(&dirs, &finfo) == FR_OK) 
             {
                if (finfo.fattrib & AM_ARC) 
...
//    nameinit(FileName);  //一开始就要调用.从目录项中得到文件名
    //影响速度
    //SD_Init();//初始化SD卡，在意外拔出之后可以正常使用
    //显示的开始坐标点
    
    if((finfo.fname[9]=='B')&&(finfo.fname[10]=='M')&&(finfo.fname[11]=='P'))
    {
        funcret=BmpDecode(finfo.fname); //得到一个BMP图像
...
//        TFT_WR_CMD(1,0x05,319);    //结束行数(0~319) 
    ...
    else if(((finfo.fname[9]=='J')&&(finfo.fname[10]=='P')&&(finfo.fname[11]=='G'))||((finfo.fname[9]=='J')&&(finfo.fname[10]=='P')&&(finfo.fname[11]=='E')))//得到JPG/JPEG图片
    {
        //得到JPEG/JPG图片的开始信息         
        res = f_open(&fsrc,finfo.fname, FA_OPEN_EXISTING | FA_READ);
        //开始时读入1024个字节到缓存里面.方便后面提取JPEG解码的信息
        f_read(&fsrc, jpg_buffer, 1024, &br);
        
        InitTable(); //初始化各个数据表 
        if((funcret=InitTag())!=FUNC_OK)return FALSE;//初始化表头不成功    
        if((SampRate_Y_H==0)||(SampRate_Y_V==0))return FALSE ;//采样率错误  
//好象没意义        AI_Drow_Init();  //初始化PICINFO.Div_Fac,启动智能画图       
...
BOOL BmpDecode(const char* BmpFileName)
{
    ...
    f_open(&fsrc, BmpFileName, FA_OPEN_EXISTING | FA_READ);                          
    f_read(&fsrc, jpg_buffer, 512, &br);  
    ...
    while(1)
    {                 
//改为1024        while(count<512)  //读取一簇512扇区 (SectorsPerClust 每簇扇区数)
        while(count<1024)    //读取1024字节
        {
...
//                if(y<=0)return TRUE; 
                ...
                if(IsElementOk(x,y,0))yok=1;//此处不改变PICINFO.staticx,y的值     
                else yok=0;      
        ...
        {                                           
            ...
            for(j=0;j<SampRate_Y_H*8;j++)
            {
                if((sizej+j)<PICINFO.ImgWidth)
                {
                    ...
                    color |=(B>>3);
                    //在这里送给LCD显示          
    //                 DrawPixel(realx,realy,color);//显示图片         
                }
                else break;
            }         
        }
