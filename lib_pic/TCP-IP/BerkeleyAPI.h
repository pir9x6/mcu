...
typedef u8 SOCKET;   //Socket descriptor
...
struct in_addr
{
    union
   {
       struct { u8 s_b1,s_b2,s_b3,s_b4; } S_un_b; // IP address in Byte
       struct { u16 s_w1,s_w2; } S_un_w; //IP address in Word
       u32 S_addr; //IP address
...
struct __attribute__((__packed__)) sockaddr_in
{
    ...
    u16    sin_port;  //Internet Protocol (IP) port.
