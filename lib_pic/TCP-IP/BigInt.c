...
extern void _masBIROM(void);
...
#if BIGINT_PROFILE
	u32 addBICounter = 0;
	u32 addBIROMCounter = 0;
	u32 subBICounter = 0;
	u32 subBIROMCounter = 0;
	u32 zeroBICounter = 0;
	u32 msbBICounter = 0;
	u32 mulBICounter = 0;
	u32 mulBIROMCounter = 0;
	u32 sqrBICounter = 0;
	u32 masBICounter = 0;
	u32 masBIROMCounter = 0;
	u32 copyBICounter = 0;
	...
	void putulhexUART(u32 dw)
	{
		...
		putcUART('x');
		while(BusyUART());
		putcUART(btohexa_high(((u8*)&dw)[3]));
		while(BusyUART());
		putcUART(btohexa_low(((u8*)&dw)[3]));
		while(BusyUART());
		putcUART(btohexa_high(((u8*)&dw)[2]));
		while(BusyUART());
		putcUART(btohexa_low(((u8*)&dw)[2]));
		while(BusyUART());
		putcUART(btohexa_high(((u8*)&dw)[1]));
		while(BusyUART());
		putcUART(btohexa_low(((u8*)&dw)[1]));
		while(BusyUART());
		putcUART(btohexa_high(((u8*)&dw)[0]));
		while(BusyUART());
		putcUART(btohexa_low(((u8*)&dw)[0]));
	}
#endif
...
/*********************************************************************
 * Function:        void BigInt(BIGINT *theInt, BIGINT_DATA_TYPE *data, u16 wWordLength)
 *
 * PreCondition:    None
 *
...
void BigInt(BIGINT *theInt, BIGINT_DATA_TYPE *data, u16 wWordLength)
{
	theInt->ptrLSB = data;
...
void BigIntROM(BIGINT_ROM *theInt, ROM BIGINT_DATA_TYPE *data, u16 wWordLength)
{
	theInt->ptrLSB = data;
	theInt->ptrMSB = data + wWordLength - 1;
...
/*********************************************************************
 * Function:        u16 BigIntMagnitude(BIGINT *n)
 *
 * PreCondition:    None
 *
 * Input:           *n: a pointer to the number
...
u16 BigIntMagnitudeROM(BIGINT_ROM *n)
{
...
void BigIntSwapEndianness(BIGINT *a)
{
	...
	// Set up the pointers
	front = (u8*)a->ptrLSB;
	end = (u8*)a->ptrMSBMax + (sizeof(BIGINT_DATA_TYPE) - 1);
