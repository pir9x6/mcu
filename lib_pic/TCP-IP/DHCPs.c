...
typedef struct
{
	u32 		LeaseExpires;	// Expiration time for this lease
...
void DHCPServerTask(void)
{
	...
	switch(smDHCPServer)
	{
		...
		case DHCP_LISTEN:
			...
			UDPGetArray((u8*)&BOOTPHeader, sizeof(BOOTPHeader));
...
static void DHCPReplyToDiscovery(BOOTP_HEADER *Header)
{
	...
	UDPPutArray((u8*)&(Header->HardwareType), 7);
	UDPPut(0x00);				// Seconds Elapsed: 0 (Not used)
	UDPPut(0x00);				// Seconds Elapsed: 0 (Not used)
	UDPPutArray((u8*)&(Header->BootpFlags), sizeof(Header->BootpFlags));
	UDPPut(0x00);				// Your (client) IP Address: 0.0.0.0 (none yet assigned)
	UDPPut(0x00);				// Your (client) IP Address: 0.0.0.0 (none yet assigned)
	UDPPut(0x00);				// Your (client) IP Address: 0.0.0.0 (none yet assigned)
	UDPPut(0x00);				// Your (client) IP Address: 0.0.0.0 (none yet assigned)
	UDPPutArray((u8*)&DHCPNextLease, sizeof(IP_ADDR));	// Lease IP address to give out
	...
	UDPPut(0x00);				// Relay Agent IP Address: 0.0.0.0 (not used)
	UDPPut(0x00);				// Relay Agent IP Address: 0.0.0.0 (not used)
	UDPPutArray((u8*)&(Header->ClientMAC), sizeof(MAC_ADDR));	// Client MAC address: Same as given by client
	...
	UDPPutArray((u8*)&AppConfig.MyMask, sizeof(IP_ADDR));
	...
	UDPPut(sizeof(IP_ADDR));
	UDPPutArray((u8*)&AppConfig.MyIPAddr, sizeof(IP_ADDR));
...
static void DHCPReplyToRequest(BOOTP_HEADER *Header, BOOL bAccept)
{
	...
	UDPPut(0x00);				// Seconds Elapsed: 0 (Not used)
	UDPPutArray((u8*)&(Header->BootpFlags), sizeof(Header->BootpFlags));
	UDPPutArray((u8*)&(Header->ClientIP), sizeof(IP_ADDR));// Your (client) IP Address:
	UDPPutArray((u8*)&DHCPNextLease, sizeof(IP_ADDR));	// Lease IP address to give out
	UDPPut(0x00);				// Next Server IP Address: 0.0.0.0 (not used)
	UDPPut(0x00);				// Next Server IP Address: 0.0.0.0 (not used)
	UDPPut(0x00);				// Next Server IP Address: 0.0.0.0 (not used)
