...
static u8 *DNSHostName;							// Host name in RAM to look up
static ROM u8 *DNSHostNameROM;					// Host name in ROM to look up
static u8 RecordType;								// Record type being queried
...
static void DNSPutString(u8* String);
static void DNSDiscardName(void);

#if defined(__18CXX)
	static void DNSPutROMString(ROM u8* String);
#else
	// Non-ROM alias for C30/C32
	#define DNSPutROMString(a)	DNSPutString((u8*)a)
#endif
...
/*****************************************************************************
  Function:
	void DNSResolve(u8* Hostname, u8 Type)
...
/*****************************************************************************
  Function:
	void DNSResolveROM(ROM u8* Hostname, u8 Type)
...
BOOL DNSIsResolved(IP_ADDR* HostIP)
{
	...
	static u8			vARPAttemptCount;
	static u8			vDNSAttemptCount;
	u8 				i;
	WORD_VAL			w;
