...
static ROM u8 _checkIpSrvrResponse[] = "Address:";
...
/****************************************************************************
  ...
  ***************************************************************************/
	...
	static ROM u16 ddnsServicePorts[] =
	{
		80,							// DYNDNS_ORG
		80,							// NO_IP_COM
		80,							// DNSOMATIC_COM
...
static u32 dwUpdateAt;		// Indicates when the next CheckIP should be done
...
void DDNSTask(void)
{
	...
	switch(smDDNS)
	{
		...
		case SM_BEGIN_CHECKIP:
			...
			{	
				MySocket = TCPOpen((u32)(ROM_PTR_BASE)DDNSClient.CheckIPServer.szROM, TCP_OPEN_ROM_HOST,
					DDNSClient.CheckIPPort, TCP_PURPOSE_DEFAULT);
			}
			else
			{
				MySocket = TCPOpen((u32)(PTR_BASE)DDNSClient.CheckIPServer.szRAM, TCP_OPEN_RAM_HOST,
					DDNSClient.CheckIPPort, TCP_PURPOSE_DEFAULT);						
			}
		...
		case SM_CHECKIP_SKT_OBTAINED:
			...
			TCPPutROMString(MySocket, (ROM u8*)"GET / HTTP/1.0\r\nHost: ");
		...
		case SM_CHECKIP_FIND_DELIMITER:
			...
			wPos = TCPFindROMArray(MySocket, (ROM u8*)"Address: ", 9, 0, FALSE);
		...
		case SM_CHECKIP_FIND_ADDRESS:
			...
			wPos = TCPFindROMArray(MySocket, (ROM u8*)"</body>", 7, 0, FALSE);
		...
		case SM_IP_UPDATE_HOME:
			...
			{
				MySocket = TCPOpen((u32)(ROM_PTR_BASE)DDNSClient.UpdateServer.szROM, TCP_OPEN_ROM_HOST, 
					DDNSClient.UpdatePort, TCP_PURPOSE_DEFAULT);
			}
			else
			{
				MySocket = TCPOpen((u32)(PTR_BASE)DDNSClient.UpdateServer.szRAM, TCP_OPEN_RAM_HOST,
					DDNSClient.UpdatePort, TCP_PURPOSE_DEFAULT);
			}
		...
		case SM_IP_UPDATE_REQ_A:
			...
			TCPPutROMString(MySocket, (ROM u8*)"GET /nic/update?hostname=");
			smDDNS++;
		...
		case SM_IP_UPDATE_REQ_C:
			...
			TCPPutROMString(MySocket, (ROM u8*)"&myip=");
			TCPPutString(MySocket, vBuffer);
			TCPPutROMString(MySocket, (ROM u8*)"&wildcard=NOCHG&mx=NOCHG&backmx=NOCHG HTTP/1.0");	
		...
		case SM_IPUPDATE_FIND_RESPONSE:
			...
			wPos = TCPFindROMArray(MySocket, (ROM u8*)"\r\n\r\n", 4, 0, FALSE);
...
void DDNSSetService(DDNS_SERVICES svc)
{
	...
	DDNSClient.UpdateServer.szROM = (ROM u8*)ddnsServiceHosts[svc];
