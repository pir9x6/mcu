...
static void BankSel(u16 Register);
static REG ReadETHReg(unsigned char Address);
static REG ReadMACReg(unsigned char Address);
static void WriteReg(unsigned char Address, unsigned char Data);
static void BFCReg(unsigned char Address, unsigned char Data);
static void BFSReg(unsigned char Address, unsigned char Data);
...
void MACInit(void)
{
    ...
    //BankSel(ERXFCON);
    //WriteReg((u8)ERXFCON, ERXFCON_CRCEN);

    // Enter Bank 2 and configure the MAC
    BankSel(MACON1);

    // Enable the receive portion of the MAC
    WriteReg((unsigned char)MACON1, MACON1_TXPAUS | MACON1_RXPAUS | MACON1_MARXEN);

    // Pad packets to 60 bytes, add CRC, and check Type/Length field.
#if defined(FULL_DUPLEX)
    WriteReg((unsigned char)MACON3, MACON3_PADCFG0 | MACON3_TXCRCEN | MACON3_FRMLNEN | MACON3_FULDPX);
    WriteReg((unsigned char)MABBIPG, 0x15);
#else
    WriteReg((unsigned char)MACON3, MACON3_PADCFG0 | MACON3_TXCRCEN | MACON3_FRMLNEN);
    WriteReg((unsigned char)MABBIPG, 0x12);
#endif

    // Allow infinite deferals if the medium is continuously busy
    // (do not time out a transmission if the half duplex medium is
    // completely saturated with other people's data)
    WriteReg((unsigned char)MACON4, MACON4_DEFER);

    // Late collisions occur beyond 63+8 bytes (8 bytes for preamble/start of frame delimiter)
    // 55 is all that is needed for IEEE 802.3, but ENC28J60 B5 errata for improper link pulse
    // collisions will occur less often with a larger number.
    WriteReg((unsigned char)MACLCON2, 63);

    // Set non-back-to-back inter-packet gap to 9.6us.  The back-to-back
    // inter-packet gap (MABBIPG) is set by MACSetDuplex() which is called
    // later.
    WriteReg((unsigned char)MAIPGL, 0x12);
    WriteReg((unsigned char)MAIPGH, 0x0C);

    // Set the maximum packet size which the controller will accept
    WriteReg((unsigned char)MAMXFLL, LOW(6+6+2+1500+4));  // 1518 is the IEEE 802.3 specified limit
    WriteReg((unsigned char)MAMXFLH, HIGH(6+6+2+1500+4)); // 1518 is the IEEE 802.3 specified limit

    // Enter Bank 3 and initialize physical MAC address registers
    BankSel(MAADR1);
    WriteReg((unsigned char)MAADR1, AppConfig.MyMACAddr.v[0]);
    WriteReg((unsigned char)MAADR2, AppConfig.MyMACAddr.v[1]);
    WriteReg((unsigned char)MAADR3, AppConfig.MyMACAddr.v[2]);
    WriteReg((unsigned char)MAADR4, AppConfig.MyMACAddr.v[3]);
    WriteReg((unsigned char)MAADR5, AppConfig.MyMACAddr.v[4]);
    WriteReg((unsigned char)MAADR6, AppConfig.MyMACAddr.v[5]);

    // Disable the CLKOUT output to reduce EMI generation
    WriteReg((unsigned char)ECOCON, 0x00);   // Output off (0V)
    //WriteReg((u8)ECOCON, 0x01); // 25.000MHz
    //WriteReg((u8)ECOCON, 0x03); // 8.3333MHz (*4 with PLL is 33.3333MHz)

    // Get the Rev ID so that we can implement the correct errata workarounds
    ENCRevID = ReadETHReg((unsigned char)EREVID).Val;
    ...
    // communication mode should be set.
    {
        ...
        BankSel(MACON3);
        Register = ReadMACReg((unsigned char)MACON3);
        Register.MACON3bits.FULDPX = PhyReg.PHCON1bits.PDPXMD;
        WriteReg((unsigned char)MACON3, Register.Val);

        // Set the back-to-back inter-packet gap time to IEEE specified
        // requirements.  The meaning of the MABBIPG value changes with the duplex
        // state, so it must be updated in this function.
        // In full duplex, 0x15 represents 9.6us; 0x12 is 9.6us in half duplex
        WriteReg((unsigned char)MABBIPG, PhyReg.PHCON1bits.PDPXMD ? 0x15 : 0x12);
    }
#endif
...
u16 MACGetFreeRxSize(void)
{
    ...
    do {
        ...
        ReadPT.v[0] = ReadETHReg((unsigned char)EPKTCNT).Val;
        ...
        BankSel(EPKTCNT);
    } while(ReadETHReg((unsigned char)EPKTCNT).Val != ReadPT.v[0]);
...
BOOL MACGetHeader(MAC_ADDR *remote, unsigned char* type)
{
    ...
    BankSel(EPKTCNT);
    PacketCount = ReadETHReg((unsigned char)EPKTCNT).Val;
    BankSel(ERDPTL);
    if(PacketCount == 0u)
        return FALSE;
 ...
 *                  important.
 *****************************************************************************/
void MACPutHeader(MAC_ADDR *remote, unsigned char type, u16 dataLen)
{
    ...
    dataLen += (u16)sizeof(ETHER_HEADER) + TXSTART;
    ...
    MACPutArray((unsigned char*)&AppConfig.MyMACAddr, sizeof(AppConfig.MyMACAddr));

    // Write the appropriate Ethernet Type u16 for the protocol being used
    MACPut(0x08);
...
u16 CalcIPBufferChecksum(u16 len)
{
    ...
    while(len)
    {
        ...
        MACGetArray((unsigned char*)DataBuffer, ChunkLen);
        ...
        {
            ((unsigned char*)DataBuffer)[ChunkLen] = 0x00;
            ChunkLen++;
        }
    ...
    Checksum.Val = (u32)Checksum.w[0] + (u32)Checksum.w[1];
...
/******************************************************************************
 * Function:        void MACMemCopyAsync(PTR_BASE destAddr, PTR_BASE sourceAddr, u16 len)
 *
...
void MACMemCopyAsync(PTR_BASE destAddr, PTR_BASE sourceAddr, u16 len)
{
    ...
    if(destAddr == (PTR_BASE)-1)
    {
        ...
        ((unsigned char*)&destAddr)[1] = ReadETHReg(EWRPTH).Val;
    }
    if(sourceAddr == (PTR_BASE)-1)
    {
        UpdateReadPointer = TRUE;
        sourceAddr = ReadETHReg(ERDPTL).Val;
        ((unsigned char*)&sourceAddr)[1] = ReadETHReg(ERDPTH).Val;
    }
    ...
    if(len <= 1u)
    {
        ...
        WriteReg(ERDPTL, ((unsigned char*)&sourceAddr)[0]);
        WriteReg(ERDPTH, ((unsigned char*)&sourceAddr)[1]);
        WriteReg(EWRPTL, ((unsigned char*)&destAddr)[0]);
        WriteReg(EWRPTH, ((unsigned char*)&destAddr)[1]);
        while(len--)
            MACPut(MACGet());
        if(!UpdateReadPointer)
        {
    ...
    else
    {
        ...
        WriteReg(EDMASTL, ((unsigned char*)&sourceAddr)[0]);
        WriteReg(EDMASTH, ((unsigned char*)&sourceAddr)[1]);
        WriteReg(EDMADSTL, ((unsigned char*)&destAddr)[0]);
        WriteReg(EDMADSTH, ((unsigned char*)&destAddr)[1]);
        if((sourceAddr <= RXSTOP) && (len > RXSTOP)) //&& (sourceAddr >= RXSTART))
            len -= RXSIZE;
        WriteReg(EDMANDL, ((unsigned char*)&len)[0]);
        WriteReg(EDMANDH, ((unsigned char*)&len)[1]);
        ...
        if(UpdateReadPointer)
        {
            ...
            if((sourceAddr <= RXSTOP) && (len > RXSTOP)) //&& (sourceAddr >= RXSTART))
                len -= RXSIZE;
            WriteReg(ERDPTL, ((unsigned char*)&len)[0]);
            WriteReg(ERDPTH, ((unsigned char*)&len)[1]);
        }
    }
}
...
unsigned char MACGet()
{
    ...
    #if defined(__C32__)
    {
        ...
        WaitForDataByte();          // Wait until u16 is transmitted
 ...
 * Note:            None
 *****************************************************************************/
u16 MACGetArray(unsigned char *val, u16 len)
{
    u16 i;
    ...
    #if defined(__C32__)
    {
        ...
        if(len >= 4)
        {
            ...
            while(1)
            {
                ENC_SSPBUF = 0x00000000;    // Send a dummy u32 to generate 32 clocks
                i += 4;
                WaitForDataByte();         // Wait until u32 is transmitted
                dwv.Val = ENC_SSPBUF;
                if(val)
                {
                    *(++val) = dwv.v[3];
    ...
    #elif defined(__C30__)
    {
        ...
        if(len >= 2)
        {
            ...
            while(1)
            {
                ENC_SSPBUF = 0x0000;    // Send a dummy u16 to generate 32 clocks
                i += 2;
                WaitForDataByte();      // Wait until u16 is transmitted
                wv.Val = ENC_SSPBUF;
                if(val)
...
void MACPut(unsigned char val)
{
    ...
    #if defined(__C32__)
    {
        ...
        ENC_SPICON1bits.MODE16 = 1;
        ENC_SSPBUF = (WBM<<8) | (u16)val;  // Start sending the u16
        WaitForDataByte();                  // Wait until u16 is transmitted
        ENC_SPICON1bits.MODE16 = 0;
    }
    #elif defined(__C30__)
 ...
 * Note:            None
 *****************************************************************************/
void MACPutArray(unsigned char *val, u16 len)
{
    volatile unsigned char Dummy;
    ...
    #if defined(__C32__)
    {
        ...
        if(len >= 4)
        {
            ...
            while(1)
            {
                ENC_SSPBUF = dwv.Val;       // Start sending the u32
                len -= 4;
                if(len < 4)
                    break;
                dwv.v[3] = *val++;
    ...
    #elif defined(__C30__)
    {
        ...
        if(len >= 2)
        {
            ...
            while(1)
            {
                ENC_SSPBUF = wv.Val;        // Start sending the u16
                len -= 2;
                if(len < 2)
                    break;
                wv.v[1] = *val++;
...
/******************************************************************************
 * Function:        REG ReadETHReg(u8 Address)
 *
 * PreCondition:    SPI bus must be initialized (done in MACInit()).
...
/******************************************************************************
 * Function:        REG ReadMACReg(u8 Address)
 * PreCondition:    SPI bus must be initialized (done in MACInit()).
 *                  Bank select bits must be set corresponding to the register
 *                  to read from.
...
PHYREG ReadPHYReg(unsigned char Register)
{
    ...
    WriteReg((unsigned char)MICMD, MICMD_MIIRD);
    ...
    // Stop reading
    BankSel(MIREGADR);
    WriteReg((unsigned char)MICMD, 0x00);

    // Obtain results and return
    Result.VAL.v[0] = ReadMACReg((unsigned char)MIRDL).Val;
    Result.VAL.v[1] = ReadMACReg((unsigned char)MIRDH).Val;
...
static void WriteReg(unsigned char Address, unsigned char Data)
{
    ...
    #if defined(__C32__)
    {
        // Send the Write Buffer Memory and data, in on 16-bit write
        ENC_SPICON1bits.MODE16 = 1;
        ENC_SSPBUF = ((WCR | Address)<<8) | (u16)Data; // Start sending the u16
        WaitForDataByte();                  // Wait until u16 is transmitted
        ENC_SPICON1bits.MODE16 = 0;
    }
    #else
...
/******************************************************************************
 * Function:        void BFCReg(u8 Address, unsigned char Data)
 * PreCondition:    SPI bus must be initialized (done in MACInit()).
 *                  Bank select bits must be set corresponding to the register
 *                    to modify.
 ...
 *****************************************************************************/
void WritePHYReg(unsigned char Register, u16 Data)
{
...
{
    BankSel(ECOCON);
    WriteReg((unsigned char)ECOCON, NewConfig);
    BankSel(ERDPTL);
}
...
unsigned char GetCLKOUT(void)
{
    ...
    BankSel(ECOCON);
    i = ReadETHReg((unsigned char)ECOCON).Val;
    BankSel(ERDPTL);
    return i;
}
...
void SetRXHashTableEntry(MAC_ADDR DestMACAddr)
{
    ...
    if((DestMACAddr.v[0] | DestMACAddr.v[1] | DestMACAddr.v[2] | DestMACAddr.v[3] | DestMACAddr.v[4] | DestMACAddr.v[5]) == 0x00u)
    {
        ...
        BFCReg((unsigned char)ERXFCON, ERXFCON_HTEN);
        for(i = (unsigned char)EHT0; i <= (unsigned char)EHT7; i++)
            WriteReg(i, 0x00);
    }
    else
    {
        ...
        BFSReg((unsigned char)ERXFCON, ERXFCON_HTEN);
    }
