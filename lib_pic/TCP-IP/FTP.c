...
static u8             FTPUser[FTP_USER_NAME_LEN];
static u8             FTPString[MAX_FTP_CMD_STRING_LEN+2];
static u8             FTPStringLen;
static u8             *FTP_argv[MAX_FTP_ARGS];    // Parameters for a ftp command
static u8             FTP_argc;       // Total number of params for a ftp command
static u32            lastActivity;   // Timeout keeper.
...
static FTP_COMMAND ParseFTPCommand(u8 *cmd);
static void ParseFTPString(void);
...
SM_FTP_RESPOND_Label:
		...
		TCPPutROMString(FTPSocket, (ROM u8*)FTPResponseString[FTPResponse]);
		TCPFlush(FTPSocket);
...
static BOOL ExecuteFTPCommand(FTP_COMMAND cmd)
{
    ...
    case FTP_CMD_PORT:
        FTPDataPort.v[1] = (u8)atoi((char*)FTP_argv[5]);
        FTPDataPort.v[0] = (u8)atoi((char*)FTP_argv[6]);
