...
static HTTP_COMMAND HTTPParse(u8 *string,
                              u8** arg,
                              u8* argc,
                              u8* type);
...
static void HTTPProcess(HTTP_HANDLE h)
{
    u8 httpData[MAX_HTML_CMD_LEN+1];
    HTTP_COMMAND httpCommand;
    BOOL lbContinue;
    u8 *arg[MAX_HTTP_ARGS];
    u8 argc;
    HTTP_INFO* ph;
	u16 w;
    ...
    do
    {
        ...
        case SM_HTTP_IDLE:
			...
			w = TCPFindROMArray(ph->socket, (ROM u8*)"\r\n", 2, 0, FALSE);
            if(w == 0xFFFFu)
			{
        ...
        case SM_HTTP_HEADER:
            if ( TCPIsPutReady(ph->socket) )
            {
				...
				{
                    ph->bProcess = TRUE;
					TCPPutROMArray(ph->socket, (ROM u8*)HTTP_OK_NO_CACHE_STRING, HTTP_OK_NO_CACHE_STRING_LEN);
				}
                else
				{
                    ph->bProcess = FALSE;
					TCPPutROMArray(ph->socket, (ROM u8*)HTTP_OK_STRING, HTTP_OK_STRING_LEN);
				}

                TCPPutROMString(ph->socket, httpContents[ph->fileType].typeString);
				TCPPutROMArray(ph->socket, (ROM u8*)HTTP_HEADER_END_STRING, HTTP_HEADER_END_STRING_LEN);
...
static BOOL SendFile(HTTP_INFO* ph)
{
	...
	if(ph->bProcess)					
	{
		...
		while(w)
		{
	        ...
	        if(ph->smHTTPGet != (u8)SM_HTTP_GET_VAR)
	        {
	            c = MPFSGet();
	            if(MPFSIsEOF())
	            {
...
/*********************************************************************
 * Function:        static HTTP_COMMAND HTTPParse(u8 *string,
 *                                              u8** arg,
 *                                              u8* argc,
 *                                              u8* type)
 *
