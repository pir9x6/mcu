...
/****************************************************************************
  ...
  ***************************************************************************/
	...
	typedef struct
	{
		u32 byteCount;					// How many bytes have been read so far
		u32 nextCallback;					// Byte index of the next callback
		u32 callbackID;					// Callback ID to execute, also used as watchdog timer
		u32 callbackPos;					// Callback position indicator
		u8 *ptrData;						// Points to first free byte in data
		u8 *ptrRead;						// Points to current read location
		MPFS_HANDLE file;					// File pointer for the file being served
	    MPFS_HANDLE offsets;				// File pointer for any offset info being used
		u8 hasArgs;						// True if there were get or cookie arguments
		u8 isAuthorized;					// 0x00-0x79 on fail, 0x80-0xff on pass
		HTTP_STATUS httpStatus;				// Request method/status
	    HTTP_FILE_TYPE fileType;			// File type to return with Content-Type
		u8 data[HTTP_MAX_DATA_LEN];		// General purpose data buffer
		#if defined(HTTP_USE_POST)
		u8 smPost;						// POST state machine variable
		#endif
	} HTTP_CONN;
...
#else
	#define RESERVED_HTTP_MEMORY ((u32)MAX_HTTP_CONNECTIONS * (u32)sizeof(HTTP_CONN))
#endif
...
void HTTPIncFile(ROM u8* cFile);
	...
	#define HTTPGetROMArg(a,b)	HTTPGetArg(a,(u8*)b)
#endif
...
/*****************************************************************************
  Function:
	HTTP_READ_STATUS HTTPReadPostPair(u8* cData, u16 wLen)
...
/*****************************************************************************
  Function:
	...
	void HTTPPrint_varname(u16 wParam1)
	void HTTPPrint_varname(u16 wParam1, u16 wParam2, ...)
  ...
  Description:
	...
	have u16 parameters included for each parameter passed in the dynamic
	variable.  For example, the variable "~myArray(2,6)~" will generate the
	prototype "void HTTPPrint_varname(u16, u16);".
