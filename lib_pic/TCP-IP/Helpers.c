...
/*****************************************************************************
  ...
  ***************************************************************************/
static u32 dwLFSRRandSeed = 0x41FE9F9E;	// 0x41FE9F9E corresponds to calling LFSRSeedRand(1)
u32 LFSRSeedRand(u32 dwSeed)
{
	u32 dwOldSeed;
	u8 i;
...
/*****************************************************************************
  ...
  Remarks:
	...
	random u32.  In such a case, the output may not be truly random.  
...
#if defined __18CXX	
{
	u8 ADCON0Save, ADCON2Save;
	u8 T0CONSave, TMR0HSave, TMR0LSave;
...
#else
{
	u16 AD1CON1Save, AD1CON2Save, AD1CON3Save;
	u16 T1CONSave, PR1Save;
...
void UnencodeURL(u8* URL)
{
	...
	while((Right = (u8*)strchr((char*)URL, '%')))
	{
...
BOOL StringToIPAddress(u8* str, IP_ADDR* IPAddress)
{
	...
	while((i = *str++))
	{
		...
		else if(charLen == 3u)
		{
			...
			IPAddress->v[currentOctet++] = dwVal.v[2]*((u8)100) + dwVal.v[1]*((u8)10) + dwVal.v[0];
			charLen = 0;
			dwVal.Val = 0;
			continue;
		}
	...
	IPAddress->v[3] = dwVal.v[2]*((u8)100) + dwVal.v[1]*((u8)10) + dwVal.v[0];
...
/*****************************************************************************
  Function:
	u16 Base64Decode(u8* cSourceData, u16 wSourceLen, 
						u8* cDestData, u16 wDestLen)
...
/*****************************************************************************
  Function:
	u16 Base64Encode(u8* cSourceData, u16 wSourceLen,
						u8* cDestData, u16 wDestLen)
...
/*****************************************************************************
  ...
  Description:
	Swaps the endian-ness of a u16.
...
/*****************************************************************************
  ...
  Description:
	Swaps the endian-ness of a u32.
...
/*****************************************************************************
  Function:
	u16 CalcIPChecksum(u8* buffer, u16 count)
  ...
  Precondition:
	buffer is u16 aligned (even memory address) on 16- and 32-bit PICs.
...
u16 CalcIPBufferChecksum(u16 len)
{
	...
	u16 ChunkLen;
	u8 DataBuffer[20];	// Must be an even size
	u16 *DataPtr;
...
/*****************************************************************************
  ...
  Description:
	This function rotates the bits in a 32-bit u32 left by a specific 
	number of bits.
...
/*****************************************************************************
  Function:
	void FormatNetBIOSName(u8 Name[])
...
/*****************************************************************************
  Function:
	u8 ExtractURLFields(u8 *vURL, 
						  PROTOCOLS *protocol, 
						  u8 *vUsername, u16 *wUsernameLen, 
						  u8 *vPassword, u16 *wPasswordLen, 
						  u8 *vHostname, u16 *wHostnameLen, 
						  u16 *wPort, 
						  u8 *vFilePath, u16 *wFilePathLen)
  ...
  Parameters:
	...
	wUsernameLen -
		On call\: Optional pointer to a u16 specifying the maximum length of 
		the vUsername buffer, including the null terminator character.
		
		<p>Upon return\: If wUsernameLen and vUsername are non-NULL, the 
		*wUsernameLen u16 is updated with the actual number of characters 
	...
	wPasswordLen -
		...
		*wPasswordLen u16 is updated with the actual number of characters 
	...
	vHostname - Optional pointer to a buffer to write the decoded hostname 
		...
		"192.168.0.1".	The IP address would not be decoded to a u32 (use the 
	...
	wHostnameLen -
		...
		*wHostnameLen u16 is updated with the actual number of characters 
	...
	wPort - Optional pointer to a u16 specifying the TCP or UDP port that the 
	...
	wFilePathLen -
		...
		*wFilePathLen u16 is updated with the actual number of characters 
...
u8 ExtractURLFields(u8 *vURL, PROTOCOLS *protocol, u8 *vUsername, u16 *wUsernameLen, u8 *vPassword, u16 *wPasswordLen, u8 *vHostname, u16 *wHostnameLen, u16 *wPort, u8 *vFilePath, u16 *wFilePathLen)
{
	...
	static ROM u16 		wProtocolPorts[] = { 80,     443,     1755,  554};
	u16 w, w2;
	u8 i, j;
	PROTOCOLS prot;
	u8 *temp, *temp2;
	...
	temp = (u8*)strnchr((char*)vURL, wURLLen, '\r');
	if(temp)
		wURLLen = temp - vURL;
	temp = (u8*)strnchr((char*)vURL, wURLLen, '\n');
	if(temp)
		wURLLen = temp - vURL;
	...
	for(prot = 0; (u8)prot < sizeof(wProtocolPorts)/sizeof(wProtocolPorts[0]); prot++)
	{
		...
		if((u16)(temp - vURL) == w)
		{
			...
			while(w)
			{
				...
				if((i >= 'A') && (i <= 'Z'))
					i += 'a' - 'A';
				if(i != (u8)vProtocolNames[prot][w2++])
					break;
				w--;
			}
			if(w == 0u)
	...
	if((u8)prot >= sizeof(wProtocolPorts)/sizeof(wProtocolPorts[0]))
		return 1;
	...
	else
	{
		...
		// symbol.
		temp2 = (u8*)strnchr((char*)vURL, temp - vURL, ':');
	...
	// terminator
	temp = (u8*)strnchr((char*)vURL, wURLLen, ':');
	temp2 = (u8*)strnchr((char*)vURL, wURLLen, '/');
	if(temp && temp2)
	{
		if(temp > temp2)
			temp = NULL;
...
/*****************************************************************************
  Function:
	SHORT Replace(u8 *vExpression, ROM u8 *vFind, ROM u8 *vReplacement, 
				  u16 wMaxLen, BOOL bSearchCaseInsensitive)
...
{
	u16 wExpressionLen, wFindLen, wFindLenMinusOne, wReplacementLen;
	u16 wFindCount, wReplacementsLeft;
	u8 i, j;
	u8 vFirstFindChar;
	u16 wBytesLeft;
	u8 *vDest;
	u8 *vExpressionCompare;
	ROM u8 *vFindCompare;
	u16 w;
	...
	if(bSearchCaseInsensitive)	// Convert to all lowercase if needed
		if((vFirstFindChar >= (u8)'A') && (vFirstFindChar <= (u8)'Z'))
			vFirstFindChar += 'a' - 'A';
