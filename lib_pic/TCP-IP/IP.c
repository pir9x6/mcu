...
static u16 _Identifier = 0;
static u8 IPHeaderLen;
                 ...
                 u16 *len)
{
    ...
    checksums[0] = ~CalcIPChecksum((u8*)&header, sizeof(header));

    // Calculate Options checksum too, if they are present.
    if ( optionsLen > 0u )
        checksums[1] = ~CalcIPChecksum((u8*)options, optionsLen);
    else
        checksums[1] = 0;

    CalcChecksum.Val  = CalcIPChecksum((u8*)checksums,
                                            2 * sizeof(u16));
...
/*********************************************************************
 * Function: u16 IPPutHeader(NODE_INFO *remote,
 *           				  u8 protocol,
 *                			  u16 len)
 *
 ...
 *                  time.
 ********************************************************************/
u16 IPPutHeader(NODE_INFO *remote,
                 u8 protocol,
                 u16 len)
{
    IP_HEADER   header;
    ...
    header.HeaderChecksum   = CalcIPChecksum((u8*)&header, sizeof(header));
...
/*********************************************************************
 * Function:        IPSetRxBuffer(u16 Offset)
 *
