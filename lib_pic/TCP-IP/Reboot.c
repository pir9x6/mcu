...
void RebootTask(void)
{
	...
	BootloaderAddress.wChecksum = CalcIPChecksum((u8*)&BootloaderAddress, sizeof(BootloaderAddress) - sizeof(BootloaderAddress.wChecksum));
	...
	#if defined(__18CXX)
	{
		...
		wvPROD.Val = ((u16)&BootloaderAddress);
		PRODH = wvPROD.v[1];
		PRODL = wvPROD.v[0];
	}
	#endif
