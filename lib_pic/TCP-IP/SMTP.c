...
static u16 ResponseCode;

/****************************************************************************
  Section:
	SMTP Client Internal Function Prototypes
  ***************************************************************************/
static u8 *FindEmailAddress(u8 *str, u16 *wLen);
static ROM u8 *FindROMEmailAddress(ROM u8 *str, u16 *wLen);
...
void SMTPTask(void)
{
	...
	static ROM u8 *ROMStrPtr, *ROMStrPtr2;
	static u8 	*RAMStrPtr;
	static u16		wAddressLength;
	...
	switch(TransportState)
	{
		...
		case TRANSPORT_BEGIN:
			...
			else
			{
				...
				{
					SMTPClient.Server.szRAM = (u8*)strchr((char*)SMTPClient.To.szRAM, '@');
					SMTPClient.ROMPointers.Server = 0;
				}
				else if(SMTPClient.To.szROM && SMTPClient.ROMPointers.To)
				{
					SMTPClient.Server.szROM = (ROM u8*)strchrpgm((ROM char*)SMTPClient.To.szROM, '@');
				...
				if(!(SMTPClient.Server.szRAM || SMTPClient.Server.szROM))
				{
					...
					{
						SMTPClient.Server.szRAM = (u8*)strchr((char*)SMTPClient.CC.szRAM, '@');
						SMTPClient.ROMPointers.Server = 0;
					}
					else if(SMTPClient.CC.szROM && SMTPClient.ROMPointers.CC)
					{
						SMTPClient.Server.szROM = (ROM u8*)strchrpgm((ROM char*)SMTPClient.CC.szROM, '@');
						SMTPClient.ROMPointers.Server = 1;
					}
				}
				...
				if(!(SMTPClient.Server.szRAM || SMTPClient.Server.szROM))
				{
					...
					{
						SMTPClient.Server.szRAM = (u8*)strchr((char*)SMTPClient.BCC.szRAM, '@');
						SMTPClient.ROMPointers.Server = 0;
					}
					else if(SMTPClient.BCC.szROM && SMTPClient.ROMPointers.BCC)
					{
						SMTPClient.Server.szROM = (ROM u8*)strchrpgm((ROM char*)SMTPClient.BCC.szROM, '@');
						SMTPClient.ROMPointers.Server = 1;
					}
				}
			...
			switch(SMTPState)
			{
				case SMTP_HELO:
					if(SMTPClient.Username.szROM == NULL)
						TCPPutROMString(MySocket, (ROM u8*)"HELO MCHPBOARD\r\n");
					else
						TCPPutROMString(MySocket, (ROM u8*)"EHLO MCHPBOARD\r\n");
					TCPFlush(MySocket);
					SMTPState++;
					break;
				...
				case SMTP_AUTH_LOGIN:
					...
					TCPPutROMString(MySocket, (ROM u8*)"AUTH LOGIN\r\n");
					TCPFlush(MySocket);
					SMTPState++;
					break;
				...
				case SMTP_MAILFROM:
					...
					// return address.
					TCPPutROMString(MySocket, (ROM u8*)"MAIL FROM:<");
				...
				case SMTP_RCPTTOCC:
				case SMTP_RCPTTOBCC:
					TCPPutROMString(MySocket, (ROM u8*)"RCPT TO:<");
				...
				case SMTP_DATA:
					TCPPutROMString(MySocket, (ROM u8*)"DATA\r\n");
					SMTPState++;
						...
						switch(PutHeadersState)
						{
							case PUTHEADERS_FROM_INIT:
								if(SMTPClient.From.szRAM || SMTPClient.From.szROM)
								{
									...
									TCPPutROMString(MySocket, (ROM u8*)"From: ");
								}
								else
								{
							...
							case PUTHEADERS_OTHER_INIT:
								TCPPutROMArray(MySocket, (ROM u8*)"\r\n", 2);
				...
				case SMTP_DATA_BODY_INIT:
					...
					CRPeriod.Pos = NULL;
					if(RAMStrPtr)
						CRPeriod.Pos = (u8*)strstrrampgm((char*)RAMStrPtr, (ROM char*)"\r\n.");
					// No break here
