...
//static u8 _SNMPGet(void);
void _SNMPPut(u8 v);
static u8 FindOIDsInRequest(u16 pdulen);
static SNMP_ACTION ProcessHeader(PDU_INFO* pduDbPtr, char* community, u8* len);
static BOOL ProcessGetSetHeader(PDU_INFO* pduDbPtr);
static BOOL ProcessVariables(PDU_INFO* pduDbPtr,char* community, u8 len);

// Section: Routines to validate snmp request pdu elements for SNMP format
static BOOL IsValidOID(u8* oid, u8* len);
static BOOL IsValidCommunity(char* community, u8* len);
//static BOOL IsValidInt(u32* val);
static BOOL IsValidPDU(SNMP_ACTION* pdu);
u8 IsValidLength(u16* len);
//static u8 IsValidStructure(u16* dataLen);
static BOOL IsASNNull(void);

// Section: Routines to read/search OIDs,objects from the SNMP MIB database
u8 OIDLookup(PDU_INFO* pduDbPtr,u8* oid, u8 oidLen, OID_INFO* rec);
BOOL GetNextLeaf(OID_INFO* rec);
BOOL GetOIDStringByAddr(OID_INFO* rec, u8* oidString, u8* len);
BOOL GetDataTypeInfo(DATA_TYPE dataType, DATA_TYPE_INFO* info);
static void ReadMIBRecord(u32 h, OID_INFO* rec);

// Section: Global variables configuration for pdu processings
 void _SNMPDuplexInit(UDP_SOCKET socket);
void SetErrorStatus(u16 errorStatusOffset,u16 errorIndexOffset,SNMP_ERR_STATUS errorStatus,u8 errorIndex);

// Section: Routine to check if private mib object is requested by NMS.
static BOOL SNMPCheckIfPvtMibObjRequested(u8* OIDValuePtr);

// This function is used only when TRAP is enabled.
#if !defined(SNMP_TRAP_DISABLED)
BOOL GetOIDStringByID(SNMP_ID id, OID_INFO* info, u8* oidString, u8* len);
...
/****************************************************************************
  Section:
	Global Variables
  ***************************************************************************/
u16 SNMPTxOffset;	//Snmp udp buffer tx offset
static u16 SNMPRxOffset;	//Snmp udp buffer rx offset		
...
u16 msgSecrtyParamLenOffset;
/* This variable is used for gext next request for zero instance  */
BOOL getZeroInstance;

u8 appendZeroToOID;//global flag to modify OID by appending zero 	
...
BOOL SNMPTask(void)
{
    ...
    hMPFS = MPFSOpenROM((ROM u8*)SNMP_BIB_FILE_NAME);
...
/****************************************************************************
  Function:
	u8 *getSnmpV2GenTrapOid(u8 generic_trap_code,u8 *len)
  Summary:
...
u8 *getSnmpV2GenTrapOid(u8 generic_trap_code,u8 *len)
{
    static  u8 gen_trap_oid[] = {0x2b,6,1,6,3,1,1,5,1};
/*	
	static  u8 cold_trap_oid[] = {0x2b,6,1,6,3,1,1,5,1};
    static  u8 warm_start_oid = {0x2b,6,1,6,3,1,1,5,2};
    static  u8 auth_fail_oid  = {0x2b,6,1,6,3,1,1,5,5};
    static  u8 linkdown_oid   = {0x2b,6,1,6,3,1,1,5,3};
    static  u8 linkup_oid     = {0x2b,6,1,6,3,1,1,5,4};
*/
	static u8 snmptrap_oids[]  = {0x2b,6,1,6,3,1,1,4,1 }; 
...
/****************************************************************************
  ...
  Parameters:
	...
	val     - Value of var. Only value of u8, u16 or u32 can be sent.
...
BOOL SNMPNotify(SNMP_ID var, SNMP_VAL val, SNMP_INDEX index)
{
	char* pCommunity;
	u8 len;
	u8 OIDValue[OID_MAX_LEN];
	u8 OIDLen;
	static u32 varbindlen = 0;
	u8 agentIDLen;
	u8* pOIDValue;
	static u16 packetStructLenOffset = 0;
	static u16 pduStructLenOffset = 0;
	static u16 varBindStructLenOffset = 0;
	static u16 varPairStructLenOffset = 0;
	static u16 prevOffset = 0;
	u16 tempOffset = 0;
	OID_INFO rec;
	DATA_TYPE_INFO dataTypeInfo;
	u8	snmptrap_oids[]  = {0x2b,6,1,6,3,1,1,4,1 }; /* len=10 */
	u8	sysUpTime_oids[] = {0x2b,6,1,2,1,1,3}; /* len = 8 */
	...
	if((packetStructLenOffset == 0)&&(pduStructLenOffset==0))
	{
		...
		_SNMPPut(ASN_OID);
		OIDLen = (u8)sizeof(sysUpTime_oids);
		_SNMPPut((u8)(OIDLen)+1);
		...
		_SNMPPut(ASN_OID);
		OIDLen = (u8)sizeof(snmptrap_oids);
		_SNMPPut((u8)(OIDLen)+1);
...
BOOL SNMPNotify(SNMP_ID var, SNMP_VAL val, SNMP_INDEX index)
{
    ...
    u8 agentIDLen;
    u8* pOIDValue;
    u16 packetStructLenOffset;
    u16 pduStructLenOffset;
    u16 varBindStructLenOffset;
    u16 varPairStructLenOffset;
    u16 prevOffset;
    OID_INFO rec;
	DATA_TYPE_INFO dataTypeInfo;
...
static SNMP_ACTION ProcessHeader(PDU_INFO* pduDbPtr, char* community, u8* len)
{
	...
	WORD_VAL snmpMsgLen;
	u16 retlen,tempCntr=0;
	UINT8* tempPtr;
#ifdef STACK_USE_SNMPV3_SERVER
	UINT8* inDataPtr;
	u8 extraMemReqdFor16BytesBlocks;
#endif
	tempPtr=snmpMsgBuf;
	...
	*snmpMsgBuf = _SNMPGet();
    ...
    if ( (tempLen.v[0] != (u8)SNMP_V1) && ( tempLen.v[0] != (u8)SNMP_V2C )&&( tempLen.v[0] != (u8)SNMP_V3 ))
        return SNMP_ACTION_UNKNOWN;
	...
	if(pduDbPtr->snmpVersion == (u8)SNMP_V3)
	{
	...
	if((tempLen.v[0] == (u8)SNMP_V1)||(tempLen.v[0] == (u8)SNMP_V2C))
	{
		...
		if(pduDbPtr->snmpVersion==(u8)SNMP_V1 && pduDbPtr->pduType == GET_BULK_REQUEST)
			return SNMP_ACTION_UNKNOWN;
	
	    // Ask main application to verify community name against requested pdu type.
	    if(SNMPValidateCommunity((u8 *)community)==(u8)INVALID_COMMUNITY)
	        return SNMP_ACTION_UNKNOWN;
	}
...
static BOOL ProcessGetSetHeader(PDU_INFO* pduDbPtr)
{
	...
	if((pduDbPtr->snmpVersion == (u8)SNMP_V1 || pduDbPtr->snmpVersion == (u8)SNMP_V2C /*|| pduDbPtr->snmpVersion == (u8)SNMP_V3*/) &&(pduDbPtr->pduType != GET_BULK_REQUEST))
	{
	...
	else if((pduDbPtr->snmpVersion == (u8)SNMP_V2C /*|| pduDbPtr->snmpVersion == (u8)SNMP_V3*/ )&& pduDbPtr->pduType == GET_BULK_REQUEST )
	{
		...
		if(IsValidInt(&tempData.Val))
			 pduDbPtr->maxRepetitions=(u8)tempData.v[0];
		else
			return FALSE;
	}
	else 
...
/****************************************************************************
  Function:
	BOOL ProcessVariables(PDU_INFO* pduDbPtr,u8* community, u8 len)
...
static BOOL ProcessVariables(PDU_INFO* pduDbPtr,char* community, u8 len)
{	
 	u8 getbulkOverFlowFlag = FALSE;
    u8 temp =0;
    u8 OIDValue[OID_MAX_LEN];
    u8 OIDLen=0;
   	u8 varIndex =0;
	u8 errorIndex;
	u8 communityLen=0,commRetVal=0;
	u8 noOfOIDsInReq=0,tempNonRepeators=0,noOfVarToBeInResponse=0;
	u8 repeatCntr,varBindCntr;
	u8 Getbulk_N=0,Getbulk_M=0,Getbulk_R=0;/*Refer RFC 3416 Section "4.2.3" GetBulkRequest-PDU*/
	u8 oidLookUpRet=0;
	u8 templen=0;
	u8 successor=0;// 'I'th lexicographic successor 	 
	u8 *ptemp;
	u8 *ptroid;
	u8 *rxedCommunityName;
	u16 varBindStructOffset=0;
   	u16 tempTxOffset=0;
    u16 oidOffset=0;
    u16 prevOffset=0;
	u16 packetStructLenOffset=0;
	//u16 snmpV3ScopedPduOffset=0;
    u16 pduLenOffset=0;
    u16 errorStatusOffset=0;
    u16 errorIndexOffset=0;    
    u16 varStructLenOffset=0;	
	u16 prevSnmpRxOffset=0;	
...
WORD_VAL bytesAdded2Pdu;
	...
	rxedCommunityName=(u8 *)community;
...
//			}
		...
		case SM_FIND_NO_OF_RESPONSE_VARBINDS:
			...
			if(((pduDbPtr->snmpVersion == (u8)SNMP_V2C)||
				((pduDbPtr->snmpVersion == (u8)SNMP_V3))) && 
	...
	//			return FALSE;
			...
			if ( pduDbPtr->pduType != (u8)SET_REQUEST )
			{
				if ( !IsASNNull() )
				return FALSE;
			}
			...
			*/
			...
			{	
				...
				if(pduDbPtr->pduType == (u8)SET_REQUEST)
				{	
					if(commRetVal==(u8)WRITE_COMMUNITY)//If SET request, then "community==WRITE_COMMUNITY" is must.
						smSnmp=SM_FIND_OID_IN_MIB;
				}
				else 
				{	
					if(commRetVal!=(u8)INVALID_COMMUNITY)//If any GET request, then "community!=INVALID_COMMUNITY" is must (community is WRITE_COMMUNITY or READ_COMMUNITY).
						smSnmp=SM_FIND_OID_IN_MIB;
				}
		...
		case SM_FIND_OID_IN_MIB:
			...
			if(oidLookUpRet != (u8)TRUE && (pduDbPtr->pduType != GET_NEXT_REQUEST) &&
				(pduDbPtr->pduType != GET_BULK_REQUEST))
			{
				...
				if(pduDbPtr->snmpVersion == (u8)SNMP_V1)
				{
				...
				else if(((pduDbPtr->snmpVersion == (u8)SNMP_V2C)
						||( pduDbPtr->snmpVersion == (u8)SNMP_V3))
		...
		case SM_NON_REPETITIONS:
			...
			if(temp == 0u &&(pduDbPtr->pduType != SNMP_SET))
			{
				...
				if((pduDbPtr->pduType == SNMP_GET_NEXT|| pduDbPtr->pduType == SNMP_V2C_GET_BULK)&&pduDbPtr->snmpVersion == (u8)SNMP_V2C)
				{
			...
			if((pduDbPtr->pduType==GET_BULK_REQUEST) &&
			   ((pduDbPtr->snmpVersion == (u8)SNMP_V2C)||
			    (pduDbPtr->snmpVersion == (u8)SNMP_V3))&&( Getbulk_N == 0u))
			{
...
/****************************************************************************
  Function:
	u8 ProcessGetNextVar(OID_INFO* rec,PDU_INFO* pduDbPtr)
...
u8 ProcessGetNextVar(OID_INFO* rec,PDU_INFO* pduDbPtr)
{
    ...
    u8 ref;
    static SNMP_VAL v;
    static u8 varDataType;
    static u8 indexBytes;
	#ifdef STACK_USE_SNMPV3_SERVER	
    ...
    indexRec.dataType = 0;
    MPFSGet(hMPFS, (u8*)&indexRec.dataType);
...
/****************************************************************************
  Function:
	u8 ProcessGetBulkVar(OID_INFO* rec, u8* oidValuePtr, 
						   u8* oidLenPtr,u8* successor)	
  ...
  Remarks:
	None.
***************************************************************************/
u8 ProcessGetBulkVar(OID_INFO* rec, u8* oidValuePtr, u8* oidLenPtr,u8* successor,PDU_INFO* pduDbPtr)
{
    
    u8 ref,putBytes,cntr;
    u8 OIDLen;
	static u8 varDataType;
    static u8 indexBytes;
	u8 sequenceCnt=0;
	u8 sequenceRepeatCnt=0;	
	BOOL lbNextLeaf;
	SNMP_ID varID;
	OID_INFO indexRec;	
	INDEX_INFO indexInfo;
...
u8 OIDLookup(PDU_INFO* pduDbPtr,u8* oid, u8 oidLen, OID_INFO* rec)
{
	...
	u8 savedOID,tempSavedOID;
	u8 matchedCount;
	u8 snmpVer;
	u8 snmpReqType;
	u8 tempLen;
	u8* reqOidPtr;
	u8 comapreOidWithSibling=FALSE;

	WORD_VAL tempData;
    u32 hNode,tempHNode;//    MPFS hNode;
    ...
    while( 1 )
    {
		...
		if(comapreOidWithSibling==(u8)FALSE)
		{
        ...
        if ( savedOID != *reqOidPtr )
        {
			...
			if(comapreOidWithSibling==(u8)TRUE && !rec->nodeInfo.Flags.bIsSibling)
				goto DidNotFindIt;
        ...
        else
        {
            ...
            if ( !rec->nodeInfo.Flags.bIsParent )
            {
				...
				else if(snmpReqType==(u8)SNMP_GET 
				...
				else if(snmpReqType==(u8)SNMP_V2C_GET_BULK && matchedCount == 1u )
				{
					appendZeroToOID=FALSE;
				}
                goto FoundIt;
...
FoundIt:
    ...
    {	
		...
		if(snmpReqType==SNMP_GET && snmpVer==(u8)SNMP_V1)
		{
			return SNMP_NO_SUCH_NAME;
		}
		else if(snmpReqType==SNMP_GET && snmpVer==(u8)SNMP_V2C)
		{
...
DidNotFindIt:
	...
	{
		if(snmpVer==(u8)SNMP_V1)
			return SNMP_NO_SUCH_NAME;

		else if(snmpVer==(u8)SNMP_V2C)
		{	
			if(matchedCount== oidLen) //No OBJECT IDNETIFIER Prefix match
				return SNMP_NO_SUCH_INSTANCE;
			else
	...
	else if((snmpReqType==SNMP_GET_NEXT||snmpReqType==SNMP_V2C_GET_BULK) && snmpVer==(u8)SNMP_V2C)
	{
...
/****************************************************************************
  Function:
	BOOL IsValidCommunity(char* community, u8* len)
...
/****************************************************************************
  Function:
	u8 IsValidStructure(u16* dataLen)
...
u8 IsValidStructure(u16* dataLen)
{
    ...
    // As a result, we will only use lower u16 of length value.
    *dataLen = tempLen.w[0];
...
/****************************************************************************
  Function:
	void ReadMIBRecord(u32 h, OID_INFO* rec)
...
/****************************************************************************
  Function:
	u8 ProcessSetVar(PDU_INFO* pduDbPtr,OID_INFO* rec, 
					   SNMP_ERR_STATUS* errorStatus)
  ...
  Remarks:
  	None.
***************************************************************************/
u8 ProcessSetVar(PDU_INFO* pduDbPtr,OID_INFO* rec, SNMP_ERR_STATUS* errorStatus)
{
	u8 ref;
	u8 temp=0;
	u8 dataType=0;
    ...
    else
    {
        ...
        if ( errorCode == SNMP_NO_ERR )
		{
            if(!SNMPSetVar(rec->id, rec->index, (u16)SNMP_END_OF_VAR, dataValue))
			{
                errorCode = SNMP_BAD_VALUE;
			}
		}
...
/****************************************************************************
  Function:
	u8 ProcessGetVar(OID_INFO* rec, BOOL bAsOID,PDU_INFO* pduDbPtr)
...
/****************************************************************************
  Function:
	void SetErrorStatus(u16 errorStatusOffset,
                           u16 errorIndexOffset,
                           SNMP_ERR_STATUS errorStatus,
                           u8 errorIndex)
  Summary:
...
/****************************************************************************
  Function:
	u8 FindOIDsInRequest(u16 pdulen)
...
/****************************************************************************
  Function:
	BOOL SNMPCheckIfPvtMibObjRequested(u8* OIDValuePtr)
...
{
	u8 cnt=0;
	u8 pvtObjIdentifier[4]={0x2b,0x06/*dod*/,0x01/*internet*/,0x04/*private*/};
