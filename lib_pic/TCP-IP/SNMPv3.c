...
extern u16 SNMPTxOffset;
extern u16 msgSecrtyParamLenOffset;
...
BOOL Snmpv3IsValidInt(u32* val);
BOOL IsSNMPv3ValidStructure(u16* dataLen);
static u8 FindOIDsFromSnmpV3Request(u16 pdulen);

void Snmpv3ReportPdu(SNMPV3MSGDATA *dynScopedBufPtr);
void Snmpv3FreeDynAllocMem(void);
static BOOL IsSnmpv3ValidOID(u8* oid, u8* len);
static BOOL IsSnmpV3ASNNull(void);
void Snmpv3SetErrorStatus(u16 errorStatusOffset,
                           u16 errorIndexOffset,
                           SNMP_ERR_STATUS errorStatus,
                           u8 errorIndex,SNMPV3MSGDATA *dynScopedPduPutBuf);
u8 Snmpv3IsValidAuthStructure(u16* dataLen);
BOOL _Snmpv3IsValidInt(UINT8* wholeMsgPtr,u16* pos, u32* val );
BOOL _IsSNMPv3ValidStructure(UINT8* wholeMsgPtr,u16* pos, u16* dataLen );
u8 _Snmpv3IsValidAuthStructure(u16* dataLen);
...
/****************************************************************************
  Function:
	u32 Snmpv3TrackAuthEngineTimeTick(void)
  ...
  Return Values:
	timeStamp : u32 value of the timer ticks 
...
u32 Snmpv3TrackAuthEngineTimeTick(void)
{
	...
	// Find total contribution from lower u32
    dw = dwvLow.Val/(u32)TICK_SECOND;
    timeStamp = dw*100ul;
    dw = (dwvLow.Val - dw*(u32)TICK_SECOND)*100ul;		// Find fractional seconds and convert to 10ms ticks
    timeStamp += (dw+((u32)TICK_SECOND/2ul))/(u32)TICK_SECOND;

	// Itteratively add in the contribution from upper u16
...
/****************************************************************************
  Function:
	BOOL Snmpv3BufferPut(u8 val ,SNMPV3MSGDATA *putbuf)
	
  Summary:
  	Copies u8 data to dynamically allocated memory buffer.

  Description:
	The SNMPv3 stack implementation uses dynamically allocated memory buffer for
	processing of request and response packets. This routine copies the u8 data to the 
  ...
  Parameters:
  	val: u8 value to be written to the buffer
...
BOOL Snmpv3BufferPut(u8 val ,SNMPV3MSGDATA *putbuf)
{
	...
	{
		putbuf->head[putbuf->length] = (u8)val;
		putbuf->length++;
		return TRUE;
	}
	else
...
/****************************************************************************
  ...
  Summary:
  	Reads u8 data from dynamically allocated memory buffer.

  Description:
	The SNMPv3 stack implementation uses dynamically allocated memory buffer for
	processing of request and response packets. This routine reads the u8 data from 
...
***************************************************************************/	
u8 Snmpv3GetBufferData(SNMPV3MSGDATA getbuf,UINT16 pos)
{
	return (u8)(getbuf.head[pos]);
}


/****************************************************************************
  Function:
	u8 Snmpv3GetWholeMsgBufferData(UINT8* getbuf, UINT16* pos)
...
/****************************************************************************
  Function: 
  	SNMP_ACTION Snmpv3MsgProcessingModelProcessPDU(u8 inOutPdu)
...
SNMP_ACTION Snmpv3MsgProcessingModelProcessPDU(u8 inOutPdu)
{
	...
	u8 tempData=0;	
	UINT8 *ptr=NULL;
	u16 tempPos=0,snmpv3Headerlength=0;
	u8 snmpv3MsgGlobalHeaderlength=0;
	u8 snmpv3MsgAuthHedaerLength = 0;	
	  
	if(inOutPdu == SNMP_REQUEST_PDU)
	{
		tempPos=snmpMsgBufSeekPos=0x00;
		if (!_IsSNMPv3ValidStructure(gSnmpV3InPduWholeMsgBuf.snmpMsgHead,&tempPos,(u16*)&tempLen.w[0]))
		...
		if(! _Snmpv3IsValidInt(gSnmpV3InPduWholeMsgBuf.snmpMsgHead,&tempPos,(u32*)&tempLen.Val))
			return SNMP_NO_CREATION;	

		if(snmpConfigureEngineSecurityModel(tempLen.Val))
		{
			incomingPdu.securityModel= tempLen.Val;
			securityPrimitivesOfIncomingPdu.securityModel= (u8)tempLen.Val;
		}
		else
			return SNMP_NO_CREATION;
...
/****************************************************************************
  Function: 
  	SNMP_ACTION Snmpv3UserSecurityModelProcessPDU(u8 inOutPdu)
...
SNMP_ACTION Snmpv3UserSecurityModelProcessPDU(u8 inOutPdu)
{
	...
	if(inOutPdu == SNMP_REQUEST_PDU)
	{
		...
		{
			securityPrimitivesOfIncomingPdu.securityEngineID=ptr=(u8 *)malloc((size_t)tempData+5);
		...
		{
			securityPrimitivesOfIncomingPdu.securityName=ptr=(u8 *)malloc((size_t)tempData+5);
...
/****************************************************************************
  Function: 
  	SNMP_ACTION Snmpv3ScopedPduProcessing(u8 inOutPdu)
...
SNMP_ACTION Snmpv3ScopedPduProcessing(u8 inOutPdu)
{
	...
	if(inOutPdu == SNMP_REQUEST_PDU)
	{
		if ( !_Snmpv3IsValidAuthStructure((u16*)&tempLen) )
		{
			return SNMP_NO_CREATION;
		}
...
BOOL ProcessSnmpv3MsgData(PDU_INFO* pduDbPtr)
{
	...
	u8			varIndex=0;
	u8 			repeatCntr=0,varBindCntr=0;
	u8			succesor=0,tempRet=0xFF;
	UINT16	 		pduLenOffset=0;
	UINT16 	 		pduLength=0;
	u16 			errorStatusOffset=0;
	u16 			errorIndexOffset=0;
	u16 			varBindHeaderLen=0;
	u16			previousGetBufpos=0;
	u16			varBindHeaderOffset=0;
	u16			varBindHeaderOffset_2=0;
	u16			varBindHeaderOffset_3=0;
	u16			contextNameOffset=0;	
	u16		 	varStructLenOffset=0;
	u16			maxRepeatationOffset=0;
	u16			tempOffset=0;						
				...
				case SM_NON_REPETITIONS:
					...
					if(tempRet == 0u &&(pduDbPtr->pduType != SNMP_SET))
					{
						if(dynScopedBufPtr->length >= dynScopedBufPtr->maxlength)
							goto GET_BULK_OVERFLOW_ERROR;
						if(((pduDbPtr->pduType == SNMP_GET_NEXT)|| (pduDbPtr->pduType == SNMP_V2C_GET_BULK))&&pduDbPtr->snmpVersion == (u8)SNMP_V3)
						{
					...
					if((pduDbPtr->pduType==GET_BULK_REQUEST) && (pduDbPtr->snmpVersion == (u8)SNMP_V3)
														&&( Getbulk_N == 0u))
					{					
						smSnmp=SM_MAX_REPETITIONS;
					}
...
/****************************************************************************
  Function:
	BOOL _IsSNMPv3ValidStructure(UINT8* wholeMsgPtr,UINT16* pos, u16* dataLen )
...
void Snmpv3ReportPdu(SNMPV3MSGDATA *dynScopedBufPtr)
{
	u8	usmStatEngineIds[]={43,6,1,6,3,15,1,1,4,0};
	u8	reportPduLenOffset=0;	
	u8	usmLen=0,i=0;
...
/****************************************************************************
  Function:
	BOOL IsSnmpv3ValidOID(u8* oid, u8* len)
...
/****************************************************************************
  Function:
	u8 Snmpv3GetUserIndxFromUsmUserDB(u8 targetIndex)
  ...
  Return Values:
	...
	u8 - Byte value fo the index matched
...
/****************************************************************************
  ...
  Remarks:
	None.
***************************************************************************/
BOOL Snmpv3CmprTrapSecNameAndSecLvlWithUSMDb(u8 tragetIndex,u8 userTrapSecLen,
...
/****************************************************************************
  Function:
	u8 Snmpv3GetTrapSecurityLevel(STD_BASED_SNMPV3_SECURITY_LEVEL securityLevel)
...
{	
	...
	if(gSNMPv3TrapScopedPduResponseBuf.head == NULL)
	{
		...
		ptr = gSNMPv3TrapScopedPduResponseBuf.head = 
			(u8*)(malloc((size_t)(SNMP_MAX_MSG_SIZE - snmpv3Headerlength)+1));
		if(ptr == NULL)
		{
		...
		Snmpv3BufferPut((u8)(OIDLen)+1,dynTrapScopedPduBuf);
		ptr = sysUpTime_oids;
		while( OIDLen-- )
	...
	{
		ptr = (u8*)getSnmpV2GenTrapOid(gGenericTrapNotification,&OIDLen);			
		if(ptr == NULL)
		{
			//MPFSClose(hMPFS);
