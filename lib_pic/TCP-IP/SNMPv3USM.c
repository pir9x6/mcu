...
//RFC3826
void Snmpv3UsmAesEncryptDecryptInitVector(u8 inOutPdu)
{
	u8 j;
	UINT8* decryptPtr;
	UINT8* prvParamPtr;
...
u8 Snmpv3AuthenticateRxedPduForDataIntegrity(SNMPV3_REQUEST_WHOLEMSG* rxDataPtr)
{	
...
UINT8* tempPtr;
	...
	if(hashTYpe == SNMPV3_HAMC_MD5)
	{
		...
		MD5Initialize(&md5);
		MD5AddData(&md5,snmpV3UserDataBase[gSnmpv3UserDBIndex].userAuthLocalKeyHmacIpad, (u16)0x40);
		MD5AddData(&md5, rxDataPtr->wholeMsgHead, rxDataPtr->wholeMsgLen.Val);
		MD5Calculate(&md5, HmacMd5Digest);
		
		MD5Initialize(&md5);
		MD5AddData(&md5, snmpV3UserDataBase[gSnmpv3UserDBIndex].userAuthLocalKeyHmacOpad, (u16)0x40);
	...
	{
		SHA1Initialize(&md5);
		SHA1AddData(&md5,snmpV3UserDataBase[gSnmpv3UserDBIndex].userAuthLocalKeyHmacIpad, (u16)0x40);
		SHA1AddData(&md5, rxDataPtr->wholeMsgHead, rxDataPtr->wholeMsgLen.Val);
		SHA1Calculate(&md5, HmacSHADigest);
		
		SHA1Initialize(&md5);
		SHA1AddData(&md5, snmpV3UserDataBase[gSnmpv3UserDBIndex].userAuthLocalKeyHmacOpad, (u16)0x40);
...
u8 Snmpv3AuthenticateTxPduForDataIntegrity(SNMPV3_RESPONSE_WHOLEMSG* txDataPtr)
{	
UINT8* secNamePtr;
UINT8 i;
static HASH_SUM md5;
...
u8 Snmpv3AESDecryptRxedScopedPdu(/*UINT8 userDBIndex*/)
{
...
u8 extraMemReqd;
	...
	decrypted_text=(u8*)(malloc((size_t)cipherTextLen+extraMemReqd));
...
u8 Snmpv3AESEncryptResponseScopedPdu(SNMPV3_RESPONSE_WHOLEMSG* plain_text/*UINT8 userDBIndex*/)
{
...
u8 Snmpv3GetSecurityLevel(u8 userIndex)
{
...
{
	u8* inSecNamePtr=NULL;
	UINT8 reportMsgName[7]="initial";//respose is "report" 0xa8 msg
	u8  tempLen=0,i=0,temp=0;
	u8  inSecurityLevel=0;
...
UINT8* Snmpv3ComputeHmacMD5Digest(UINT8 * inData, UINT32 dataLen,UINT8* userExtendedLclzdKeyIpad,UINT8* userExtendedLclzdKeyOpad)
{
	...
	MD5Initialize(&md5);
	MD5AddData(&md5, userExtendedLclzdKeyIpad, (u16)0x40);
	MD5AddData(&md5, data2Hmac, (u16)dataLen);
	MD5Calculate(&md5, HmacMd5Digest);


	
	MD5Initialize(&md5);
	MD5AddData(&md5, userExtendedLclzdKeyOpad, (u16)0x40);
	MD5AddData(&md5, HmacMd5Digest,16);
	MD5Calculate(&md5, HmacMd5Digest);
...
{	
	...
	SHA1Initialize(&sha1);
	SHA1AddData(&sha1, authKey_iPad, (u16)0x40);
	SHA1AddData(&sha1, data2Hmac, (u16)dataLen);
	SHA1Calculate(&sha1, HmacSHADigest);


	
	SHA1Initialize(&sha1);
	SHA1AddData(&sha1, authKey_oPad, (u16)0x40);
