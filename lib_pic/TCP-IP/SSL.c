...
/****************************************************************************
  ...
  ***************************************************************************/
	...
	static u8 sslStubID;			// Which SSL_STUB is loaded
	static SSL_KEYS sslKeys;		// The current SSL session
	static u8 sslKeysID;			// Which SSL_KEYS are loaded
	static u8 sslBufferID;		// Which buffer is loaded
	static u8 sslHashID;			// Which hash is loaded
	static u8 sslSessionID;		// Which session is loaded
	static BOOL sslSessionUpdated;	// Whether or not it has been updated
	static u8 sslRSAStubID;		// Which stub is using RSA, if any
	...
	u8 *ptrHS;					// Used in buffering handshake results

	extern ROM u16 SSL_CERT_LEN;	// RSA public certificate length		?
	extern ROM u8 SSL_CERT[];		// RSA public certificate data			?
...
/****************************************************************************
  ...
  ***************************************************************************/
	...
	static void GenerateHashRounds(u8 num, u8* rand1, u8* rand2);
	static void CalculateFinishedHash(u8 hashID, BOOL fromClient, u8 *result);
	static void GenerateSessionKeys(void);

	// Section: Ethernet Buffer RAM Management
	static void SSLStubSync(u8 id);
	static BOOL SSLStubAlloc(void);
	static void SSLStubFree(u8 id);
	static void SSLKeysSync(u8 id);
	static void SSLHashSync(u8 id);
	static void SSLHashAlloc(u8 *id);
	static void SSLHashFree(u8 *id);
	static void SSLBufferSync(u8 id);
	static void SSLBufferAlloc(u8 *id);
	static void SSLBufferFree(u8 *id);
	static u8 SSLSessionNew(void);
	static void SSLSessionSync(u8 id);
	#define SSLSessionUpdated()		sslSessionUpdated = TRUE;
	static void SaveOffChip(u8 *ramAddr, PTR_BASE ethAddr, u16 len);
	static void LoadOffChip(u8 *ramAddr, PTR_BASE ethAddr, u16 len);
	
	// Section: Handshake Hash and I/O Functions
	static void HSStart(void);
	static void HSEnd(void);
	static u16 HSGet(TCP_SOCKET skt, u8 *b);
	static u16 HSGetWord(TCP_SOCKET skt, u16 *w);
	static u16 HSGetArray(TCP_SOCKET skt, u8 *data, u16 len);
	static u16 HSPut(TCP_SOCKET skt, u8 b);
	static u16 HSPutWord(TCP_SOCKET skt, u16 w);
	static u16 HSPutArray(TCP_SOCKET skt, u8 *data, u16 len);
	#if defined(STACK_USE_SSL_SERVER)
	static u16 HSPutROMArray(TCP_SOCKET skt, ROM u8 *data, u16 len);
	#endif
	
	// Section: Client messages
	#if defined(STACK_USE_SSL_CLIENT)
		static u8 SSLSessionMatchIP(IP_ADDR ip);
	...
	#if defined(STACK_USE_SSL_SERVER)
		static u8 SSLSessionMatchID(u8* SessionID);
...
/****************************************************************************
  ...
  ***************************************************************************/
u16 isStubUsed;			// Indicates which stubs are in use
u16 isHashUsed;			// Indicates which hashes are in use
u16 isBufferUsed;			// Indicates which buffers are in use

// Masks for each bit in the is*Used variables
static ROM u16 masks[16] = { 0x0001, 0x0002, 0x0004, 0x0008,
...
/*****************************************************************************
  Function:
	void SSLPeriodic(TCP_SOCKET hTCP, u8 id)
...
/*****************************************************************************
  Function:
	u8 SSLStartSession(TCP_SOCKET hTCP, u8 * buffer, u8 supDataType)
  ...
  ***************************************************************************/
u8 SSLStartSession(TCP_SOCKET hTCP, void * buffer, u8 supDataType)
{
	u8 i;
...
/*****************************************************************************
  Function:
	u16 SSLRxRecord(TCP_SOCKET hTCP, u8 id)
  ...
  Returns:
  	u16 indicating the number of data bytes there were decrypted but left in 
  	the stream.
...
{	
	...
	if(sslStub.wRxBytesRem == 0u)
	{
		...
		if((sslStub.rxProtocol & 0x80) == 0x80)
		{
			...
			((u8*)&sslStub.wRxBytesRem)[1] = sslStub.rxProtocol & 0x7F;
			TCPGet(hTCP, ((u8*)&sslStub.wRxBytesRem));
		...
		#endif
		{
			// Read version (u16, currently ignored)
			TCPGet(hTCP, NULL);
			TCPGet(hTCP, NULL);
	
			// Read length (u16)
			TCPGet(hTCP, ((u8*)&sslStub.wRxBytesRem)+1);
			TCPGet(hTCP, ((u8*)&sslStub.wRxBytesRem));
...
/*****************************************************************************
  Function:
	void SSLTxRecord(TCP_SOCKET hTCP, u8 id, u8 txProtocol)
...
/*****************************************************************************
  Function:
	void SSLStartPartialRecord(TCP_SOCKET hTCP, u8 id, u8 txProtocol,
								 u16 wLen)
  ...
  ***************************************************************************/
#if defined(STACK_USE_SSL_SERVER)
void SSLStartPartialRecord(TCP_SOCKET hTCP, u8 id, u8 txProtocol, u16 wLen)
{
	u8 hdr[5];
...
/*****************************************************************************
  Function:
	void SSLTxMessage(TCP_SOCKET hTCP, u8 id, u8 msg)
...
/*********************************************************************
 * Function:        u8 SSLTxClientHello(TCP_SOCKET hTCP)
 *
...
/*********************************************************************
 * Function:        u8 SSLRxClientHello(TCP_SOCKET hTCP)
 *
...
/*********************************************************************
 * Function:        u8 SSLRxAntiqueClientHello(TCP_SOCKET hTCP)
 *
 * PreCondition:    Handshake hasher is started, and SSL has a stub
 *					assigned.
 *
...
static void SSLRxServerHello(TCP_SOCKET hTCP)
{
	...
	// Read and verify Cipher Suite (u16)
	HSGetWord(hTCP, &w);
	if(w != SSL_RSA_WITH_ARCFOUR_128_MD5)
		TCPRequestSSLMessage(hTCP, SSL_ALERT_HANDSHAKE_FAILURE);
	
	// Read and verify Compression Method (u8)
	HSGet(hTCP, &b);
	if(b != 0x00u)
		TCPRequestSSLMessage(hTCP, SSL_ALERT_HANDSHAKE_FAILURE);
...
/*********************************************************************
 * Function:        u8 SSLTxServerHello(TCP_SOCKET hTCP)
 *
 * PreCondition:    None
 *
 * Input:           hTCP - the TCP Socket to send the message to
...
static void SSLRxServerCertificate(TCP_SOCKET hTCP)
{
	...
	switch(sslStub.dwTemp.v[0]) {
		...
		case RX_SERVER_CERT_START:
			...
			len = TCPFindROMArray(hTCP, (ROM u8*)"\x2A\x86\x48\x86\xF7\x0D\x01\x01\x01", 9, 0, FALSE);
...
/*********************************************************************
 * Function:        u8 SSLTxServerCertificate(TCP_SOCKET hTCP)
 *
 * PreCondition:    None
 *
 * Input:           hTCP - the TCP Socket to send the message to
...
/*********************************************************************
 * Function:        u8 SSLTxServerHelloDone(TCP_SOCKET hTCP)
 *
 * PreCondition:    None
 *
 * Input:           hTCP - the TCP Socket to send the message to
...
static void SSLTxClientKeyExchange(TCP_SOCKET hTCP)
{
	...
	TCPPutArray(hTCP, (u8*) sslBuffer.full + 0x80, len);
...
/*********************************************************************
 * Function:        void GenerateHashRounds(u8 num, u8* rand1,
 *												u8* rand2)
 *
 ...
 ********************************************************************/
void GenerateHashRounds(u8 num, u8* rand1, u8* rand2)
{
	u8 i, j, c, *res;
...
void GenerateSessionKeys(void)
{
	...
	if(sslStub.Flags.bIsServer)
	{
		...
		ARCFOURInitialize(&(sslKeys.Remote.app.cryptCtx), (u8*)(&sslHash), 16);
		SSLBufferSync(sslStub.idTxBuffer);
		sslKeys.Local.app.cryptCtx.Sbox = sslBuffer.full;
		ARCFOURInitialize(&(sslKeys.Local.app.cryptCtx), (u8*)(&sslHash)+16, 16);
	...
	SSLBufferSync(sslStub.idTxBuffer);
	sslKeys.Local.app.cryptCtx.Sbox = sslBuffer.full;
	ARCFOURInitialize(&(sslKeys.Local.app.cryptCtx), (u8*)(&sslHash), 16);
	SSLBufferSync(sslStub.idRxBuffer);
	sslKeys.Remote.app.cryptCtx.Sbox = sslBuffer.full;
	ARCFOURInitialize(&(sslKeys.Remote.app.cryptCtx), (u8*)(&sslHash)+16, 16);
	#endif
	
}

/*********************************************************************
 * Function:        static void CalculateFinishedHash(u8 hashID,
 *									BOOL fromClient, u8 *result)
 *
...
static void CalculateFinishedHash(u8 hashID, BOOL fromClient, u8 *result)
{
	...
	if(fromClient)
		HashAddROMData(&sslHash, (ROM u8*)"CLNT", 4);
	else
		HashAddROMData(&sslHash, (ROM u8*)"SRVR", 4);
	...
	if(sslHash.hashType == HASH_SHA1)
		i--;
	for(; i > 0u; i--)
		HashAddROMData(&sslHash, (ROM u8*)"\x36\x36\x36\x36\x36\x36\x36\x36", 8);
	...
	if(sslHash.hashType == HASH_SHA1)
		i--;
	for(; i > 0u; i--)
		HashAddROMData(&sslHash, (ROM u8*)"\x5c\x5c\x5c\x5c\x5c\x5c\x5c\x5c", 8);
...
/*********************************************************************
 * Function:        static void SSLStubFree(u8 id)
 *
 * PreCondition:    None
 *
...
/*********************************************************************
 * Function:        static void SSLStubSync(u8 id)
 *
 * PreCondition:    None
 *
...
/*********************************************************************
 * Function:        static void SSLKeysSync(u8 id)
 *
 * PreCondition:    None
 *
...
static void SSLKeysSync(u8 id)
{
	...
	if(sslKeysID != SSL_INVALID_ID)
		SaveOffChip((u8*)&sslKeys,
			SSL_BASE_KEYS_ADDR+SSL_KEYS_SIZE*sslKeysID,
			SSL_KEYS_SIZE);
	
	// Load new stub
	LoadOffChip((u8*)&sslKeys,
...
/*********************************************************************
 * Function:        static void SSLHashAlloc(u8 *id)
 *
 * PreCondition:    None
 *
...
/*********************************************************************
 * Function:        static void SSLHashFree(u8 *id)
 *
 * PreCondition:    None
 *
...
/*********************************************************************
 * Function:        static void SSLHashSync(u8 id)
 *
 * PreCondition:    None
 *
...
/*********************************************************************
 * Function:        static void SSLBufferAlloc(u8 *id)
 *
 * PreCondition:    None
 *
...
/*********************************************************************
 * Function:        static void SSLBufferFree(u8 *id)
 *
 * PreCondition:    None
 *
...
/*********************************************************************
 * Function:        static void SSLBufferSync(u8 id)
 *
 * PreCondition:    None
 *
...
/*********************************************************************
 * Function:        static u8 SSLSessionNew(void)
 *
 * PreCondition:    None
 *
 * Input:           None
...
/*********************************************************************
 * Function:        static u8 SSLSessionMatchID(u8* SessionID)
 *
 * PreCondition:    None
 *
...
/*********************************************************************
 * Function:        static u8 SSLSessionMatchHost(IP_ADDR ip)
 *
 * PreCondition:    None
 *
...
/*********************************************************************
 * Function:        static void SSLSessionSync(u8 id)
 *
 * PreCondition:    None
 *
...
/*********************************************************************
 * Function:        static void SaveOffChip(u8 *ramAddr,
 *											u16 ethAddr, u16 len)
 *
 * PreCondition:    None
 *
...
/*********************************************************************
 * Function:        static void LoadOffChip(u8 *ramAddr,
 *											u16 ethAddr, u16 len)
 *
 * PreCondition:    None
 *
...
/*********************************************************************
 * Function:        static u16 HSGet(TCP_SOCKET skt, u8 *b)
 *
 * PreCondition:    None
 *
...
/*********************************************************************
 * Function:        static u16 HSGetWord(TCP_SOCKET skt, u16 *w)
 *
 * PreCondition:    None
 *
 * Input:           skt - socket to read data from
...
{
	if(w == NULL)
		return HSGet(skt, (u8*)w) + HSGet(skt, (u8*)w);
	else
		return HSGet(skt, (u8*)w+1) + HSGet(skt, (u8*)w);
}

/*********************************************************************
 * Function:        static u16 HSGetArray(TCP_SOCKET skt, 
 											 u8 *data, u16 len)
 *
 * PreCondition:    None
 *
...
//	while(w)
//	{
//		u16 wChunkLen = sizeof(sslBuffer.full) - (ptrHS - sslBuffer.full);
//		if(wChunkLen == 0u)
//		{
//			HSEnd();
//			HSStart();
...
/*********************************************************************
 * Function:        static u16 HSPut(TCP_SOCKET skt, u8 b)
 *
 * PreCondition:    None
 *
...
/*********************************************************************
 * Function:        static u16 HSPutWord(TCP_SOCKET skt, u16 w)
 *
 * PreCondition:    None
 *
 * Input:           skt  - socket to write data to
...
static u16 HSPutWord(TCP_SOCKET skt, u16 w)
{
	return HSPut(skt, (u8)(w>>8)) + HSPut(skt, (u8)w);
}

/*********************************************************************
 * Function:        static u16 HSPutArray(TCP_SOCKET skt,
 											u8 *data, u8 len)
 *
 * PreCondition:    None
 *
...
/*********************************************************************
 * Function:        static void SSLMACBegin(u8 *MACSecret, u32 seq, 
 *											u8 protocol, u16 len)
 *
 ...
 * Note:            None
 ********************************************************************/
void SSLMACBegin(u8 *MACSecret, u32 seq, u8 protocol, u16 len)
{
	u8 i, temp[7];
	...
	HashAddROMData(&sslHash, (ROM u8*)"\0\0\0\0", 4);
	HashAddData(&sslHash, temp, 7);	
}

/*********************************************************************
 * Function:        void SSLMACAdd(u8 *data, u16 len)
 *
 * PreCondition:    sslHash is ready to be written
 ...
 * Note:            None
 ********************************************************************/
void SSLMACAdd(u8 *data, u16 len)
{
	HashAddData(&sslHash, data, len);
}

/*********************************************************************
 * Function:        static void SSLMACCalc(u8 *result)
 *
 * PreCondition:    sslHash is ready to be written
 *					(any pending data saved, SSLMACBegin called)
 *
 ...
 * Note:            None
 ********************************************************************/
void SSLMACCalc(u8 *MACSecret, u8 *result)
{
	u8 i;
