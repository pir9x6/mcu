...
/****************************************************************************
  ...
  ***************************************************************************/
	...
	typedef struct
	{
		u16 wRxBytesRem;					// Bytes left to read in current record
		u16 wRxHsBytesRem;					// Bytes left to read in current Handshake submessage
		
		u8 rxProtocol;					// Protocol for message being read
		u8 rxHSType;						// Handshake message being received
		
		u8 idSession;						// ID for associated session
		u8 idMD5, idSHA1;					// ID for current hashes
		u8 idRxHash;						// ID for MAC hash (TX needs no persistence)
		u8 idRxBuffer, idTxBuffer;		// ID for current buffers (Sboxes)
		...
		u8 requestedMessage;				// Currently requested message to send, or 0xff
        void * supplementaryBuffer;
        u8 supplementaryDataType;
	} SSL_STUB;
    ...
    typedef struct
    {
      ...
      u8 pub_key[128];
      u8 pub_e[3];
      u8 pub_guid;    // This is used as a TCP_SOCKET which is a u8
    } SSL_PKEY_INFO;
	...
	typedef struct
	{
		union
		{
			struct
			{
				u8 MACSecret[16];			// Server's MAC write secret
				u32 sequence;				// Server's write sequence number
				ARCFOUR_CTX cryptCtx;		// Server's write encryption context
				u8 reserved[8];			// Future expansion
			}app;
			u8 random[32];				// Server.random value
		} Local;
		...
		union
		{
			struct
			{
				u8 MACSecret[16];			// Client's MAC write secret
				u32 sequence;				// Client's write sequence number
				ARCFOUR_CTX cryptCtx;		// Client's write encryption context
				u8 reserved[8];			// Future expansion
			}app;
			u8 random[32];				// Client.random value
		} Remote;		
	} SSL_KEYS;
	...
	typedef union
	{
		struct
		{
			...
			u8 md5_hash[16];
			u8 sha_hash[20];
			u8 temp[256-sizeof(HASH_SUM)-16-20];
		} hashRounds;
		u8 full[256];
	} SSL_BUFFER;
	...
	typedef struct
	{
		u8 sessionID[32];					// The SSL Session ID for this session
		u8 masterSecret[48];				// Associated Master Secret for this session
	} SSL_SESSION;
	...
	typedef struct
	{
		...
		u32 lastUsed;						// Tick count when session was last used
	} SSL_SESSION_STUB;

	
	#define SSL_STUB_SIZE		((u32)sizeof(SSL_STUB))				// Amount of space needed by a single SSL stub
	#define SSL_STUB_SPACE		(SSL_STUB_SIZE*MAX_SSL_CONNECTIONS)		// Amount of space needed by all SSL stubs
	#define SSL_KEYS_SIZE		((u32)sizeof(SSL_KEYS))				// Amount of space needed by a single SSL key
	#define SSL_KEYS_SPACE		(SSL_KEYS_SIZE*MAX_SSL_CONNECTIONS)		// Amount of space needed by all SSL key
	#define SSL_HASH_SIZE		((u32)sizeof(HASH_SUM))				// Amount of space needed by a single SSL hash
	#define SSL_HASH_SPACE		((u32)(SSL_HASH_SIZE*MAX_SSL_HASHES))	// Amount of space needed by all SSL hash
	#define SSL_BUFFER_SIZE		((u32)sizeof(SSL_BUFFER))				// Amount of space needed by a single SSL buffer
	#define SSL_BUFFER_SPACE	(SSL_BUFFER_SIZE*MAX_SSL_BUFFERS)		// Amount of space needed by all SSL buffer
	#define SSL_SESSION_SIZE	((u32)sizeof(SSL_SESSION))			// Amount of space needed by a single SSL session
	...
	#define RESERVED_SSL_MEMORY ((u32)(SSL_STUB_SPACE + SSL_KEYS_SPACE + SSL_HASH_SPACE + SSL_BUFFER_SPACE + SSL_SESSION_SPACE))
...
void SSLTerminate(u8 sslStubId);
void SSLPeriodic(TCP_SOCKET hTCP, u8 sslStubID);
u16 SSLRxRecord(TCP_SOCKET hTCP, u8 sslStubID);
void SSLRxHandshake(TCP_SOCKET hTCP);
void SSLTxRecord(TCP_SOCKET hTCP, u8 sslStubID, u8 txProtocol);
void SSLTxMessage(TCP_SOCKET hTCP, u8 sslStubID, u8 msg);
...
void SSLMACCalc(u8* MACSecret, u8* result);
	...
	void SSLStartPartialRecord(TCP_SOCKET hTCP, u8 sslStubID, u8 txProtocol, u16 wLen);
