...
typedef struct __attribute__((__packed__)) 
{
	...
	u8		NetBIOSName[16];        // NetBIOS name
	struct
	{
		unsigned char : 6;
...
#if defined(WF_CS_TRIS)
	u8		MySSID[32];             // Wireless SSID (if using MRF24WB0M)
	u8        SsidLength;             // number of bytes in SSID
	u8        SecurityMode;           // WF_SECURITY_OPEN or one of the other security modes
	u8        SecurityKey[64];        // WiFi Security key, or passphrase.   
	u8        SecurityKeyLength;      // number of bytes in security key (can be 0)
	u8        WepKeyIndex;            // WEP key index (only valid for WEP)
    #if defined(EZ_CONFIG_STORE) // WLAN configuration data stored to NVM
    u8        dataValid;
    u8        networkType;
    u8        saveSecurityInfo;       // Save 32-byte PSK
    #endif
#endif
	...
	u8 readCommunity[SNMP_MAX_COMMUNITY_SUPPORT][SNMP_COMMUNITY_MAX_LEN+1]; 

	// SNMPv2C Write community names
	// SNMP_COMMUNITY_MAX_LEN (8) + 1 null termination byte
	u8 writeCommunity[SNMP_MAX_COMMUNITY_SUPPORT][SNMP_COMMUNITY_MAX_LEN+1];
