...
void TCPTXPerformanceTask(void)
{
	...
	static u32 dwTimeStart;
	static u32 dwBytesSent;
	static DWORD_VAL dwVLine;
	u8 vBuffer[10];
	static u8 vBytesPerSecond[12];
	u16 w;
	u32 dw;
	u32 qw;
	...
	while(w >= 12+27+5+32u)
	{
		...
		TCPPutROMString(MySocket, (ROM u8*)": We are currently achieving ");
		TCPPutROMArray(MySocket, (ROM u8*)"       ", 5-strlen((char*)vBytesPerSecond));
		TCPPutString(MySocket, vBytesPerSecond);
		TCPPutROMString(MySocket, (ROM u8*)"00 bytes/second TX throughput.\r\n");
	...
	{
//		qw = ((u32)dwBytesSent) * (TICK_SECOND/100/8);
//		qw = ((u32)dwBytesSent) * (TICK_SECOND/100);
//		qw /= dw;
		qw = ((u32)dwBytesSent)*(TICK_SECOND/100) + (dw>>1);
		qw /= dw;
		ultoa((u32)qw, vBytesPerSecond);
		dwTimeStart += dw;
...
void TCPRXPerformanceTask(void)
{
	...
	if(dw > TICK_SECOND)
	{
		...
		qw = (u32)dwBytesRead * TICK_SECOND;
		qw += dw>>1;
		qw /= dw;
		ultoa((u32)qw, vBuffer);
		TCPPutString(MySocket, vBuffer);
		TCPPutROMString(MySocket, (const u8*)" bytes/second\r\n");
