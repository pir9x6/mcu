...
static u32			_linkUpdTick;						// last tick value when the link update was started
...
void MACInit(void)
{
    union
    {
        ...
        u8		addr[6];            // address itself
    }SysMACAddr;        // aligned MAC address

    int		ix;
	eEthRes		ethRes, phyInitRes;
	u8		useFactMACAddr[6] = {0x00, 0x04, 0xa3, 0x00, 0x00, 0x00};		// to check if factory programmed MAC address needed
	u8		unsetMACAddr[6] =   {0x00, 0x00, 0x00, 0x00, 0x00, 0x00};		// not set MAC address
...
/******************************************************************************
 * Function:        void MACPutArray(u8* buff, u16 len)
 *
 * PreCondition:    None
 *
 ...
 * Note:            None
 *****************************************************************************/
void MACPutArray(u8 *buff, u16 len)
{
...
/******************************************************************************
 * Function:        u16 MACGetArray(u8 *address, u16 len)
 *
 * PreCondition:    A valid packet should vahe been obtained or the read pointer properly set.
 *
 ...
 *****************************************************************************/
u16 MACGetArray(u8 *address, u16 len)
{
	if(address)
	{
...
void SetRXHashTableEntry(MAC_ADDR DestMACAddr)
{
      ...
      u8                      nullMACAddr[6] =   {0x00, 0x00, 0x00, 0x00, 0x00, 0x00};
