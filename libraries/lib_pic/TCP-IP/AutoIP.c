...
void AutoIPRandSeed (u32 seed, u8 vInterface);
u32 AutoIPRand (u8 vInterface);
    ...
    for (i = 0; i < NETWORK_INTERFACES; i++)
    {
        ...
        switch (AutoIPClient.smAUTOIPState)
        {
            ...
            case SM_AUTOIP_INIT_RNG:
                AutoIPRandSeed (((u32)AppConfig.MyMACAddr.v[0] + ((u32)AppConfig.MyMACAddr.v[1] << 8) + \
                        ((u32)AppConfig.MyMACAddr.v[2] << 16) + ((u32)AppConfig.MyMACAddr.v[3] << 24) + \
                        ((u32)AppConfig.MyMACAddr.v[4]) + ((u32)AppConfig.MyMACAddr.v[5] << 8)), i);
            ...
            case SM_AUTOIP_GRATUITOUS_ARP3:
                ...
                if (TickGet() - AutoIPClient.eventTime > AutoIPClient.randomDelay)
                {
                    ...
                    MACPutArray((u8*)&AutoIPClient.packet, sizeof(AutoIPClient.packet));
                    MACFlush();
