...
static u16 gAutoPortNumber = 1024;
...
int sendto( SOCKET s, const char* buf, int len, int flags, const struct sockaddr* to, int tolen )
{
	...
	static u32 startTick;		// NOTE: startTick really should be a per socket BSDSocket structure member since other BSD calls can interfere with the ARP cycles
	u16 wRemotePort;
	...
	else if(socket->SocketType == SOCK_STREAM) //TCP will only send to the already established socket.
	{
		...
		size = TCPPutArray(socket->SocketID, (u8*)buf, len);
		if(size)
			return size;
	}
	return SOCKET_ERROR;
...
int recv( SOCKET s, char* buf, int len, int flags )
{
	...
	if(socket->SocketType == SOCK_STREAM) //TCP
	{
		...
		return TCPGetArray(socket->SocketID, (u8*)buf, len);
	}
