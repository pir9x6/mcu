...
typedef struct
{
	...
	u32 				dwTimer;		// Tick timer value used for triggering future events after a certain wait period.
	u32				dwLeaseTime;	// DHCP lease time remaining, in seconds
	u32				dwServerID;		// DHCP Server ID cache
...
static u8 _DHCPReceive(void);
static void _DHCPSend(u8 messageType, BOOL bRenewing);


/*****************************************************************************
  Function:
	static void LoadState(u8 vInterface)
...
/*****************************************************************************
  Function:
	void DHCPDisable(u8 vInterface)
...
/*****************************************************************************
  Function:
	BOOL DHCPIsEnabled(u8 vInterface)
...
/*****************************************************************************
  Function:
	BOOL DHCPIsBound(u8 vInterface)
...
/*****************************************************************************
  Function:
	BOOL DHCPStateChanged(u8 vInterface)
...
/*****************************************************************************
  Function:
	BOOL DHCPIsServerDetected(u8 vInterface)
...
static u8 _DHCPReceive(void)
{
	...
	********************************************************************/
	...
	if ( v == BOOT_REPLY )
	{
		...
		{
			UDPSetRxBuffer(16);
			UDPGetArray((u8*)&DHCPClient.tempIPAddress, sizeof(DHCPClient.tempIPAddress));
			...
			switch(v)
			{
				...
				case DHCP_SUBNET_MASK:
					...
					if ( v == 4u )
					{
						...
						{
							UDPGetArray((u8*)&DHCPClient.tempMask, sizeof(DHCPClient.tempMask));
				...
				case DHCP_ROUTER:
					...
					if ( j >= 4u )
					{
						...
						{
							UDPGetArray((u8*)&DHCPClient.tempGateway, sizeof(DHCPClient.tempGateway));
				...
				case DHCP_DNS:
					...
					if(!DHCPClient.flags.bits.bOfferReceived)
					{
						UDPGetArray((u8*)&DHCPClient.tempDNS, sizeof(DHCPClient.tempDNS));
					...
					if(j >= 4u)
					{
						...
						if(!DHCPClient.flags.bits.bOfferReceived)
						{
							UDPGetArray((u8*)&DHCPClient.tempDNS2, sizeof(DHCPClient.tempDNS2));
				...
				case DHCP_SERVER_IDENTIFIER:
					...
					if ( v == 4u )
					{
						UDPGet(&(((u8*)&tempServerID)[3]));   // Get the id
						UDPGet(&(((u8*)&tempServerID)[2]));
						UDPGet(&(((u8*)&tempServerID)[1]));
						UDPGet(&(((u8*)&tempServerID)[0]));
					}
					else
						goto UDPInvalid;
					break;
				...
				case DHCP_IP_LEASE_TIME:
					...
					if ( v == 4u )
					{
						...
						{
							UDPGet(&(((u8*)(&DHCPClient.dwLeaseTime))[3]));
							UDPGet(&(((u8*)(&DHCPClient.dwLeaseTime))[2]));
							UDPGet(&(((u8*)(&DHCPClient.dwLeaseTime))[1]));
							UDPGet(&(((u8*)(&DHCPClient.dwLeaseTime))[0]));
...
static void _DHCPSend(u8 messageType, BOOL bRenewing)
{
	...
	{
		UDPPutArray((u8*)&DHCPClient.tempIPAddress, sizeof(DHCPClient.tempIPAddress));
	}
	else
	{
		UDPPut(0x00);
	...
	UDPPutArray((u8*)&AppConfig.MyMACAddr, sizeof(AppConfig.MyMACAddr));
	...
	if((messageType == DHCP_REQUEST_MESSAGE) && !bRenewing)
	{
		...
		UDPPut(((u8*)(&DHCPClient.dwServerID))[3]);
		UDPPut(((u8*)(&DHCPClient.dwServerID))[2]);
		UDPPut(((u8*)(&DHCPClient.dwServerID))[1]);
		UDPPut(((u8*)(&DHCPClient.dwServerID))[0]);
	}
		...
		((messageType == DHCP_DISCOVER_MESSAGE) && DHCPClient.tempIPAddress.Val))
	{
		...
		UDPPutArray((u8*)&DHCPClient.tempIPAddress, DHCP_PARAM_REQUEST_IP_ADDRESS_LEN);
	}
