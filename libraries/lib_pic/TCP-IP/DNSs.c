...
void DNSServerTask(void)
{
	...
	// Read DNS header
	UDPGetArray((u8*)&DNSHeader, sizeof(DNSHeader));
	...
	UDPPutArray((u8*)&DNSHeader.wTransactionID, 2);	// 2 byte Transaction ID
	...
	UDPPut(0x04);
	UDPPutArray((u8*)&AppConfig.MyIPAddr.Val, 4);	// Our IP address
