...
/****************************************************************************
  ...
  ***************************************************************************/
		...
		static void HTTPLoadConn(u8 hHTTP);
	#endif
	HTTP_STUB httpStubs[MAX_HTTP_CONNECTIONS];	// HTTP stubs with state machine and socket
	u8 curHTTPID;								// ID of the currently loaded HTTP_CONN
	#if defined(__18CXX) && !defined(HI_TECH_C)	
		#pragma udata
	#endif
...
/****************************************************************************
  Section:
	Function Prototypes
  ***************************************************************************/
	static void HTTPHeaderParseLookup(u8 i);
	...
	static HTTP_READ_STATUS HTTPReadTo(u8 delim, u8* buf, u16 len);
	#endif
...
void HTTPInit(void)
{
    ...
    for(curHTTPID = 0; curHTTPID < MAX_HTTP_CONNECTIONS; curHTTPID++)
    {
		...
		curHTTP.offsets = MPFS_INVALID_HANDLE;
			...
			MACPutArray((u8*)&curHTTP, sizeof(HTTP_CONN));
...
static void HTTPLoadConn(u8 hHTTP)
{
	...
	MACGetArray((u8*)&curHTTP, sizeof(HTTP_CONN));
...
static void HTTPProcess(void)
{
    ...
    BOOL isDone;
	u8 *ext;
	u8 buffer[HTTP_MAX_HEADER_LEN+1];
    ...
    do
    {
		...
		case SM_HTTP_SERVE_HEADERS:
			...
			// Send headers
			TCPPutROMString(sktHTTP, (ROM u8*)HTTPResponseHeaders[curHTTP.httpStatus]);
			...
			if(curHTTP.httpStatus == HTTP_REDIRECT)
			{
				...
				TCPPutROMString(sktHTTP, (ROM u8*)"\r\n\r\n304 Redirect: ");
				TCPPutString(sktHTTP, curHTTP.data);
				TCPPutROMString(sktHTTP, (ROM u8*)HTTP_CRLF);
			}
			...
			{
				TCPPutROMString(sktHTTP, (ROM u8*)"Content-Type: ");
				TCPPutROMString(sktHTTP, (ROM u8*)httpContentTypes[curHTTP.fileType]);
			...
			{
				TCPPutROMString(sktHTTP, (ROM u8*)"Content-Encoding: gzip\r\n");
			}
						
			// Output the cache-control
			TCPPutROMString(sktHTTP, (ROM u8*)"Cache-Control: ");
			if(curHTTP.httpStatus == HTTP_POST || curHTTP.nextCallback != 0xffffffff)
			{// This is a dynamic page or a POST request, so no cache
				TCPPutROMString(sktHTTP, (ROM u8*)"no-cache");
			}
			else
			{// This is a static page, so save it for the specified amount of time
				TCPPutROMString(sktHTTP, (ROM u8*)"max-age=");
				TCPPutROMString(sktHTTP, (ROM u8*)HTTP_CACHE_LEN);
			}
			TCPPutROMString(sktHTTP, HTTP_CRLF);
		...
		case SM_HTTP_SERVE_COOKIES:
			...
			{
				// Write the header
				TCPPutROMString(sktHTTP, (ROM u8*)"Set-Cookie: ");
...
static void HTTPHeaderParseCookie(void)
{
	...
	if(lenB >= (u16)(curHTTP.data + HTTP_MAX_DATA_LEN - curHTTP.ptrData - 2))
	{// If not, overflow
		curHTTP.httpStatus = HTTP_OVERFLOW;
		smHTTP = SM_HTTP_SERVE_HEADERS;
		return;
...
/*****************************************************************************
  Function:
	u8* HTTPGetArg(u8* cData, u8* cArg)
...
/*****************************************************************************
  Function:
	u8* HTTPGetROMArg(u8* cData, ROM u8* cArg)
...
/*****************************************************************************
  Function:
	HTTP_READ_STATUS HTTPReadPostName(u8* cData, u16 wLen)
...
/*****************************************************************************
  Function:
	HTTP_READ_STATUS HTTPReadPostValue(u8* cData, u16 wLen)
...
/*****************************************************************************
  Function:
	static HTTP_READ_STATUS HTTPReadTo(u8 cDelim, u8* cData, u16 wLen)
...
static HTTP_IO_RESULT HTTPMPFSUpload(void)
{
	...
	switch(curHTTP.httpStatus)
	{
		...
		case HTTP_MPFS_UP:
			...
			lenA = TCPFindROMArray(sktHTTP, (ROM u8*)"\r\n\r\n", 4, 0, FALSE);
