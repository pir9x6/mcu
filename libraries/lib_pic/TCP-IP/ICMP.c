...
static u16 wICMPSequenceNumber;
// ICMP tick timer variable
static u32 ICMPTimer;
 ...
 * Note:            None
 ********************************************************************/
void ICMPProcess(NODE_INFO *remote, u16 len)
{
	DWORD_VAL dwVal;

    // Obtain the ICMP header Type, Code, and Checksum fields
    MACGetArray((u8*)&dwVal, sizeof(dwVal));
	...
	if(dwVal.w[0] == 0x0008u)
	{
		...
		MACPutArray((u8*)&dwVal, sizeof(dwVal));
...
#if defined(STACK_USE_ICMP_CLIENT)
/*********************************************************************
 * Function:        void ICMPSendPing(u32 dwRemoteIP)
 *
 ...
 * Note:            None
 ********************************************************************/
void ICMPSendPing(u32 dwRemoteIP)
{
...
/*********************************************************************
 * Function:        void ICMPSendPingToHost (u8 * szRemoteHost)
 *
 * PreCondition:    ICMPBeginUsage() returned TRUE
 *
 ...
 * Note:            None
 ********************************************************************/
void ICMPSendPingToHost(u8 * szRemoteHost)
{
	ICMPFlags.bReplyValid = 0;
	ICMPTimer = TickGet();
...
/*********************************************************************
 * Function:        void ICMPSendPingToHostROM (ROM u8 * szRemoteHost)
 *
 * PreCondition:    ICMPBeginUsage() returned TRUE
 *
 * Input:           szRemoteHost: Host name to ping.  Must be stored 
 ...
 * Note:            None
 ********************************************************************/
void ICMPSendPingToHostROM(ROM u8 * szRemoteHost)
{
	ICMPFlags.bReplyValid = 0;
	ICMPTimer = TickGet();
...
LONG ICMPGetReply(void)
{
	...
	switch(ICMPState)
	{
		...
		case SM_ICMP_SEND_ECHO_REQUEST:
			...
			ICMPPacket.wChecksum = CalcIPChecksum((u8*)&ICMPPacket, sizeof(ICMPPacket));
			...
			MACPutArray((u8*)&ICMPPacket, sizeof(ICMPPacket));
			MACFlush();
