 ...
 */
	...
	{
	    u8 Flag __attribute__((__packed__));
	    MPFS Address __attribute__((__packed__));
	    u8 Name[MAX_FILE_NAME_LEN] __attribute__((__packed__));
	} MPFS_ENTRY;
	...
	#if defined(__C30__)
		static u32 ReadProgramMemory(u32 address);
	#endif
#endif
...
/*********************************************************************
 * Function:        MPFS MPFSOpen(u8* file)
 *
 * PreCondition:    None
 *
...
MPFS MPFSOpen(u8* file)
{
    ...
    while(1)
    {
        ...
        #elif defined(MPFS_USE_SPI_FLASH)
        	SPIFlashReadArray(FAT, (u8*)&entry, sizeof(entry));
		#else
			#if defined(__C30__)
	        	memcpypgm2ram(&entry, (ROM void*)(u16)FAT, sizeof(entry));
			#else
...
MPFS MPFSOpenROM(ROM u8* file) 
{
	u8 nameRAM[MAX_FILE_NAME_LEN+1];
...
u8 MPFSGet(void)
{
    u8 t;
		...
		_currentHandle++;
	#elif defined(MPFS_USE_SPI_FLASH)
		SPIFlashReadArray(_currentHandle++, (u8*)&t, 1);
	#else
		#if defined(__C30__)
			{
				DWORD_VAL i;
...
/*********************************************************************
 * Function:        static u32 ReadProgramMemory(u32 address)
 *
 * PreCondition:    None
 *
...
{  
	...
	wTBLPAGSave = TBLPAG;
	TBLPAG = ((u16*)&address)[1];
	((u16*)&dwResult)[1] = __builtin_tblrdh((u16)address);
	((u16*)&dwResult)[0] = __builtin_tblrdl((u16)address);
	TBLPAG = wTBLPAGSave;
...
/*********************************************************************
 * Function:        BOOL MPFSGetLong(u32 *ul)
 *
 * PreCondition:    MPFSOpen() and MPFSBeginGet()
 *
 * Input:           ul: pointer to an u32 to read
 *
 ...
 * Side Effects:    None
 *
 * Overview:        Reads an u32 value from an MPFS file
 *
 * Note:            None
 ********************************************************************/
BOOL MPFSGetLong(u32 *ul)
{
	u8* b = (u8*)ul;
	*(b) = MPFSGet();	
	if(MPFSIsEOF())
