...
#define __MPFS_H
    ...
    typedef u32 MPFS;									// For default 32 bit addressing (use /ll option with MPFS.exe)
    #define MPFS_INVALID				(0xffffffffu)	// For default 32 bit addressing (use /ll option with MPFS.exe)
//    typedef u16 MPFS;								// For old short 16 bit addressing (use /b option with MPFS.exe)
...
BOOL MPFSInit(void);
MPFS MPFSOpen(u8* name);
MPFS MPFSOpenROM(ROM u8* name);
u8 MPFSGet(void);
void MPFSClose(void);
...
#if !defined(__MPFS_C)
	extern u8 mpfsOpenCount;
    extern MPFS _currentHandle;
    extern u8 _currentCount;
#endif
