 ...
 * MPFS Structure:
 *     [M][P][F][S]
 *     [u8 Ver Hi][u8 Ver Lo][u16 Number of Files]
 ...
 *     [u32 String Ptr][u32 Data Ptr]
 *     [u32 Len][u32 Timestamp][u32 Microtime]
 *     [u16 Flags]
 *
...
static u16 fatCacheID;

// Number of files in this MPFS image
static u16 numFiles;


static void _LoadFATRecord(u16 fatID);
...
/****************************************************************************
  ...
  ***************************************************************************/
	...
	// An address where MPFS data starts in program memory.
  		...
  		extern ROM u8 MPFS_Start[];
	    #define MPFS_HEAD		((u32)(&MPFS_Start[0]))
    #else
	  	extern u32 MPFS_Start;
...
MPFS_HANDLE MPFSOpen(u8* cFile)
{
	...
	for(i = 0; i < numFiles; i++)
	{
		...
		if((i & 0x07) == 0u)
		{
			...
			MPFSGetArray(0, (u8*)hashCache, 16);
		}
...
/*****************************************************************************
  Function:
	MPFS_HANDLE MPFSOpenROM(ROM u8* cFile) 
...
/*****************************************************************************
  Function:
	MPFS_HANDLE MPFSOpenID(u16 hFatID)
...
/*****************************************************************************
  Function:
	BOOL MPFSGet(MPFS_HANDLE hMPFS, u8* c)
...
BOOL MPFSGet(MPFS_HANDLE hMPFS, u8* c)
{
    ...
    // Read function for EEPROM
		...
		*c = XEERead();
		...
		#else
		{
			u32 dwHITECHWorkaround = MPFS_HEAD;
	  	*c = *((ROM u8*)(MPFSStubs[hMPFS].addr+dwHITECHWorkaround));
		    MPFSStubs[hMPFS].addr++;
		}
		#endif
	#endif
...
/*****************************************************************************
  Function:
	u16 MPFSGetArray(MPFS_HANDLE hMPFS, u8* cData, u16 wLen)
...
/*****************************************************************************
  Function:
	BOOL MPFSGetLong(MPFS_HANDLE hMPFS, u32* ul)

  Description:
	Reads a u32 or Long value from the MPFS.
  ...
  Parameters:
	hMPFS - the file handle from which to read
	ul - where to store the u32 or long value that was read
...
BOOL MPFSGetLong(MPFS_HANDLE hMPFS, u32* ul)
{
	return ( MPFSGetArray(hMPFS, (u8*)ul, 4) == 4u );
}

/*****************************************************************************
  Function:
	BOOL MPFSSeek(MPFS_HANDLE hMPFS, u32 dwOffset, MPFS_SEEK_MODE tMode)
...
MPFS_HANDLE MPFSFormat(void)
{
	...
	isMPFSLocked = TRUE;
		...
		MPFSStubs[0].bytesRem = MPFS_WRITE_PAGE_SIZE - ( ((u8)MPFSStubs[0].addr+MPFS_HEAD) & (MPFS_WRITE_PAGE_SIZE-1) );
...
/*****************************************************************************
  Function:
	u16 MPFSPutArray(MPFS_HANDLE hMPFS, u8 *cData, u16 wLen)
...
static void _LoadFATRecord(u16 fatID)
{
	...
	MPFSGetArray(0, (u8*)&fatCache, 22);
	fatCacheID = fatID;
}

/*****************************************************************************
  Function:
	u32 MPFSGetTimestamp(MPFS_HANDLE hMPFS)
  ...
  Returns:
	The timestamp that was read as a u32
  ***************************************************************************/
u32 MPFSGetTimestamp(MPFS_HANDLE hMPFS)
{
	// Make sure a valid file is open
...
/*****************************************************************************
  Function:
	u32 MPFSGetMicrotime(MPFS_HANDLE hMPFS)
  ...
  Returns:
	The microtime that was read as a u32
  ***************************************************************************/
u32 MPFSGetMicrotime(MPFS_HANDLE hMPFS)
{
	// Make sure a valid file is open
...
/*****************************************************************************
  Function:
	u16 MPFSGetFlags(MPFS_HANDLE hMPFS)
...
/*****************************************************************************
  Function:
	u32 MPFSGetSize(MPFS_HANDLE hMPFS)
  ...
  Returns:
	The size that was read as a u32
  ***************************************************************************/
u32 MPFSGetSize(MPFS_HANDLE hMPFS)
{
	// Make sure a valid file is open
...
/*****************************************************************************
  Function:
	u32 MPFSGetBytesRem(MPFS_HANDLE hMPFS)
  ...
  Returns:
	The number of bytes remaining in the file as a u32
  ***************************************************************************/
u32 MPFSGetBytesRem(MPFS_HANDLE hMPFS)
{
	// Make sure a valid file is open
...
/*****************************************************************************
  Function:
	BOOL MPFSGetFilename(MPFS_HANDLE hMPFS, u8* cName, u16 wLen)
...
/*****************************************************************************
  Function:
	u32 MPFSGetPosition(MPFS_HANDLE hMPFS)
  ...
  Returns:
	The position in the file as a u32 (or MPFS_PTR)
...
/*****************************************************************************
  Function:
	u16 MPFSGetID(MPFS_HANDLE hMPFS)
...
static void _Validate(void)
{
	...
	MPFSGetArray(0, (u8*)&fatCache, 6);
	if(!memcmppgm2ram((void*)&fatCache, (ROM void*)"MPFS\x02\x01", 6))
		MPFSGetArray(0, (u8*)&numFiles, 2);
	else
		numFiles = 0;
