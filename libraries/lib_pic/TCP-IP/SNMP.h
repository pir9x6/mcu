...
typedef union
{
	...
	u8 Val;						   //MIB file access byte flag
} SNMP_STATUS;
...
typedef union 
{
	...
	u8 Val;					//value is OID/index byte flag
} INDEX_INFO;
...
typedef union 
{
	...
	u8 Val;								//MIB Obj info as byte value
} MIB_INFO;
...
typedef struct 
{
u16 noSuchObjectErr;		//Var list no such obj errors flags
u16 noSuchNameErr; 		//Var list no such name error 
u16 noSuchInstanceErr; 	//Var list no such instance error
u16 endOfMibViewErr;		//Var list end of mib view error
}reqVarErrStatus;  
...
typedef struct 
{
	u8 asnType;	//ASN data type
	u8 asnLen;	//ASN data length
} DATA_TYPE_INFO;
...
typedef struct 
{
	...
	u8 communityLen;						//Community name length
	SNMP_ID agentIDVar; 					//Agent id for trap identification
	u8 notificationCode;					//Trap notification code
...
typedef struct 
{
	u32			hNode;		//Node location in the mib		
	u8			oid;		//Object Id
	...
	u32			hData;		//Data
	u32			hSibling;	//Sibling info
	u32			hChild; 	//Child info
	u8			index;		//Index of object
	u8			indexLen;	//Index length
} OID_INFO;
...
typedef struct 
{
	...
	u8			nonRepeators;	//# non repeaters in the request	
	u8			maxRepetitions; //# max repeaters in the request 
	u8			pduType;		//Snmp pdu type
	u8			errorStatus;	//Pdu error status
	u8			erroIndex;		//Pdu error Index
	u8			snmpVersion;	//Snmp version
	u16			pduLength;		//Pdu length
} PDU_INFO;
...
typedef union
{
    u32 dword;				//double word value
    u16  word;					//word value
    u8  byte;					//byte value
    u8  v[sizeof(u32)];		//byte array
} SNMP_VAL;
...
/****************************************************************************
  Section:
	Global Variables
  ***************************************************************************/
extern u8 gSendTrapFlag;//Global flag to send Trap
extern u8 gGenericTrapNotification;//Global flag for Generic trap notification
extern u8 gSpecificTrapNotification;//Global flag for vendor specific trap notification
extern u8 gOIDCorrespondingSnmpMibID;//Gloabal var to store SNMP ID of var for OID received in SNMP request.
extern u8	gSetTrapSendFlag;
extern u8 appendZeroToOID;//global flag to modify OID by appending zero	
...
u8 SNMPValidateCommunity(u8* community);
BOOL SNMPNotify(SNMP_ID var, SNMP_VAL val, SNMP_INDEX index);
BOOL SNMPSetVar(SNMP_ID var, SNMP_INDEX index,u8 ref, SNMP_VAL val);
BOOL SNMPGetVar(SNMP_ID var, SNMP_INDEX index,u8* ref, SNMP_VAL* val);
BOOL SNMPIsNotifyReady(IP_ADDR* remoteHost);
void SNMPNotifyPrepare(IP_ADDR* remoteHost, char* community, u8 communityLen, SNMP_ID agentIDVar, u8 notificationCode, u32 timestamp);
BOOL SNMPGetNextIndex(SNMP_ID var, SNMP_INDEX* index);
BOOL SNMPGetExactIndex(SNMP_ID var, SNMP_INDEX index);
BOOL SNMPIsValidSetLen(SNMP_ID var, u8 len,u8 index);
...
extern u8 IsValidStructure(u16* dataLen);
extern BOOL IsValidInt(u32* val);
extern u8 _SNMPGet(void);
extern void _SNMPPut(u8 v);

extern 	void SetErrorStatus(u16 errorStatusOffset,u16 errorIndexOffset, SNMP_ERR_STATUS errorStatus,u8 errorIndex);
extern u8 OIDLookup(PDU_INFO* pduDbPtr,u8* oid, u8 oidLen, OID_INFO* rec);
extern 	BOOL GetNextLeaf(OID_INFO* rec);
extern u8 IsValidLength(u16 *len);

extern BOOL SNMPIdRecrdValidation(PDU_INFO* pduDbPtr,OID_INFO *var,u8 * oidValuePtr,u8 oidlen);



extern SNMP_ACTION Snmpv3MsgProcessingModelProcessPDU(u8 inOutPdu);
extern SNMP_ACTION Snmpv3UserSecurityModelProcessPDU(u8 inOutPdu);
extern SNMP_ACTION Snmpv3ScopedPduProcessing(u8 inOutPdu);

extern UINT8 Snmpv3TrapScopedpdu(SNMP_ID var, SNMP_VAL val, SNMP_INDEX index,UINT8 targetIndex);
extern BOOL GetOIDStringByID(SNMP_ID id, OID_INFO* info, u8* oidString, u8* len);


extern void _SNMPDuplexInit(UDP_SOCKET socket);
extern void Snmpv3GetAuthEngineTime(void);
extern void Snmpv3UsmAesEncryptDecryptInitVector(u8 inOutPdu);
extern void Snmpv3UsmOutMsgAuthenticationParam(UINT8 hashType);

extern u8 Snmpv3AESDecryptRxedScopedPdu(/*UINT8 userDBIndex*/);
extern u8 *getSnmpV2GenTrapOid(u8 generic_trap_code,u8 *len);
extern u8 Snmpv3GetBufferData(SNMPV3MSGDATA getbuf,UINT16 pos);
...
extern BOOL Snmpv3BufferPut(u8 val ,SNMPV3MSGDATA *putbuf);
