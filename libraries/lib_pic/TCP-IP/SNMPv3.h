...
{
	u8* contextEngineID; //take responsibility for this one
	u8 pduType;			//the pduType(s) to be registered
}statusInformation; //success or errorIndication

struct unregisterContextEngineID 
{
	u8* contextEngineID; //give up responsibility for this one
	u8 pduType;			//the pduType(s) to be unregistered
};
...
{
	u8 transportDomain;		//transport domain to be used
	UINT32 transportAddress;	//transport address to be used
	u8 messageProcessingModel;//typically, SNMP version
	u8 securityModel; 		//Security Model to use
	u8* securityName; 		//on behalf of this principal
	u8 securityLevel;			//Level of Security requested
	u8* contextEngineID; 	//data from/at this entity
	u8* contextName;			//data from/in this context
	u8 pduVersion; 			//the version of the PDU
	u8* PDU; 					//SNMP Protocol Data Unit
...
{
	u8 messageProcessingModel;	//typically, SNMP version
	u8 securityModel;				//Security Model in use
	u8* securityName;			//on behalf of this principal
	u8 securityLevel;				//Level of Security
	u8* contextEngineID;			//data from/at this SNMP entity
	u8* contextName;				//data from/in this context
	u8 pduVersion;				//the version of the PDU
	u8* PDU;						//SNMP Protocol Data Unit
...
//	IN securityParameters; 			//for the received message
	...
	u8* wholeMsg; 				//as received on the wire
	u8* securityEngineID;	 		//authoritative SNMP entity
	u8* securityName; 			//identification of the principal
	u8* scopedPDU; 				//message (plaintext) payload
//	OUT securityStateReference; 	//reference to security state
	UINT32 maxSizeResponseScopedPDU;//maximum size sender can handle
	u8 messageProcessingModel; 	//typically, SNMP version
	u8 securityModel; 			//for the received message
	u8 securityLevel; 			//Level of Security
	UINT8 securityEngineIDLen;	 		//authoritative SNMP entity
	u8 securityNameLength;
...
struct dispathcerReturnResponsePdu
{
	...
	u8 securityModel;				//Security Model in use
	u8* securityName;			//on behalf of this principal
	u8 securityLevel;				//same as on incoming request
	u8* contextEngineID;			//data from/at this SNMP entity
	u8* contextName;				//data from/in this context
	u8 pduVersion;				//the version of the PDU
	u8* PDU;						//SNMP Protocol Data Unit
...
struct MsgProcModPrepareOutgoingMessage
{
	...
	u8 destTransportDomain;	//destination transport domain
	UINT32 destTransportAddress;//destination transport address
	u8* outgoingMessage;		//the message to send
...
{
	u8 messageProcessingModel;	//typically, SNMP version
	u8 securityModel;  			//same as on incoming request
	u8* securityName;  			//same as on incoming request
	u8 securityLevel;  			//same as on incoming request
	u8* contextEngineID;  		//data from/at this SNMP entity
	u8* contextName;  			//data from/in this context
	u8 pduVersion;  				//the version of the PDU
...
{
	u8 transportDomain;		//origin transport domain
	UINT32 transportAddress; 	//origin transport address
	u8* wholeMsg; 			//as received from the network
...
{
	u8 securityModel; 	//Security Model in use
	u8* securityName;	//principal who wants to access
	u8 securityLevel;	 	//Level of Security
	u8 viewType; 			//read, write, or notify view
	u8* contextName; 	//context containing variableName
	u8* variableName; 	//OID for the managed object
};
...
{
	u8 messageProcessingModel; 	//typically, SNMP version
	u8* globalData; 				//message header, admin data
	UINT32 maxMessageSize; 			//of the sending SNMP entity
	u8 securityModel; 			//for the outgoing message
	u8* securityEngineID ; 		//authoritative SNMP entity
	u8* securityName;			//on behalf of this principal
	u8 securityLevel; 			//Level of Security requested
	u8* scopedPDU; 				//message (plaintext) payload
	//OUT securityParameters; 		//filled in by Security Module
	u8* wholeMsg ; 				//complete generated message
...
struct SecuritySysGenerateResponseMsg
{
	...
	u8* securityEngineID; 		//authoritative SNMP entity
	u8* securityName;			//on behalf of this principal
	u8 securityLevel; 			//for the outgoing message
	u8* scopedPDU; 				//message (plaintext) payload
...
{	
	...
	WORD_VAL snmpMsgLen;
	u16 msgAuthParamOffsetInWholeMsg;
	u16 scopedPduOffset;
	u8 scopedPduAuthStructVal;
	u16 scopedPduStructLen;
...
extern BOOL Snmpv3CmprTrapSecNameAndSecLvlWithUSMDb(u8 tragetIndex,u8 userTrapSecLen,
			UINT8 *userTrapSecurityName,STD_BASED_SNMPV3_SECURITY_LEVEL securityLevel);

extern u8 Snmpv3GetTrapSecurityLevel(STD_BASED_SNMPV3_SECURITY_LEVEL securityLevel);
extern u8 Snmpv3GetSecurityLevel(u8 userIndex);

extern u8 Snmpv3AESEncryptResponseScopedPdu(SNMPV3_RESPONSE_WHOLEMSG* plain_text/*UINT8 userDBIndex*/);
extern u8 Snmpv3AuthenticateTxPduForDataIntegrity(SNMPV3_RESPONSE_WHOLEMSG* txDataPtr);
extern u8 Snmpv3AuthenticateRxedPduForDataIntegrity(SNMPV3_REQUEST_WHOLEMSG* rxDataPtr);
