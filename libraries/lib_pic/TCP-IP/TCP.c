...
#define TCP_START_TIMEOUT_VAL   	((u32)TICK_SECOND*1)	// Timeout to retransmit unacked data
#define TCP_DELAYED_ACK_TIMEOUT		((u32)TICK_SECOND/10)	// Timeout for delayed-acknowledgement algorithm
#define TCP_FIN_WAIT_2_TIMEOUT		((u32)TICK_SECOND*5)	// Timeout for FIN WAIT 2 state
#define TCP_KEEP_ALIVE_TIMEOUT		((u32)TICK_SECOND*10)	// Timeout for keep-alive messages when no traffic is sent
#define TCP_CLOSE_WAIT_TIMEOUT		((u32)TICK_SECOND/5)	// Timeout for the CLOSE_WAIT state
...
#define TCP_SYN_QUEUE_TIMEOUT		((u32)TICK_SECOND*3)	// Timeout for when SYN queue entries are deleted if unserviceable
...
typedef struct
{
	u16    SourcePort;		// Local port number
	u16    DestPort;		// Remote port number
	u32   SeqNumber;		// Local sequence number
	u32   AckNumber;		// Acknowledging remote sequence number
	...
	u16    Window;			// Local free RX buffer window
	u16    Checksum;		// Data payload checksum
	u16    UrgentPointer;	// Urgent pointer
} TCP_HEADER;
...
typedef struct 
{
	...
	u16		wSourcePort;	// Remote TCP port number that the response SYN needs to be sent to
	u32		dwSourceSEQ;	// Remote TCP SEQuence number that must be ACKnowledged when we send our response SYN
	u16		wDestPort;		// Local TCP port which the original SYN was destined for
	u16		wTimestamp;		// Timer to expire old SYN packets that can't be serviced at all
} TCP_SYN_QUEUE;


#if defined(STACK_CLIENT_MODE)
static u16 NextPort __attribute__((persistent));	// Tracking variable for next local client port number
#endif
...
static void TCPRAMCopy(PTR_BASE wDest, u8 vDestType, PTR_BASE wSource, u8 vSourceType, u16 wLength);

#if defined(__18CXX)
	static void TCPRAMCopyROM(PTR_BASE wDest, u8 wDestType, ROM u8* wSource, u16 wLength);
#else
	#define TCPRAMCopyROM(a,b,c,d)	TCPRAMCopy(a,b,c,TCP_PIC_RAM,d)
#endif

static void SendTCP(u8 vTCPFlags, u8 vSendFlags);
static void HandleTCPSeg(TCP_HEADER* h, u16 len);
...
void TCPInit(void)
{
	...
	u16 wCurrentSPIAddress = TCP_SPI_RAM_BASE_ADDRESS;
		...
		// cycles the device.
		if(NextPort == 0u)
			NextPort = (((u16)GenerateRandomDWORD()) & 0x07FFu) + LOCAL_PORT_START_NUMBER;
	#endif
...
/*****************************************************************************
  ...
  Input:
    ...
    vRemoteHostType -  Any one of the following flags to identify the
                       ...
                       * TCP_OPEN_RAM_HOST &#45; Open a client socket and
                         ...
                         "www.microchip.com" or "192.168.0.123" (u8&#42;
                         type)
                       ...
                       * TCP_OPEN_IP_ADDRESS &#45; Open a client socket and
                         ...
                         for 192.168.1.123 (u32 type). Note that the byte
  ...
  Example:
    ...
    skt = TCPOpen((u32)(PTR_BASE)"www.microchip.com",
    ...
    skt = TCPOpen((u32)(PTR_BASE)&amp;cache.remote, TCP_OPEN_NODE_INFO,
			...
			#if defined(STACK_CLIENT_MODE)
			{
				...
				switch(vRemoteHostType)
				{
					...
					case TCP_OPEN_NODE_INFO:
						...
						memcpy((void*)(u8*)&MyTCB.remote, (void*)(u8*)(PTR_BASE)dwRemoteHost, sizeof(NODE_INFO));
...
u16 TCPIsPutReady(TCP_SOCKET hTCP)
{
	...
	if(!( (i == (u8)TCP_ESTABLISHED) || (i == (u8)TCP_CLOSE_WAIT) ))
		return 0;
...
BOOL TCPPut(TCP_SOCKET hTCP, u8 byte)
{
	...
	else if(!MyTCBStub.Flags.bTimer2Enabled)
	{
		...
		MyTCBStub.eventTime2 = (u16)TickGetDiv256() + TCP_AUTO_TRANSMIT_TIMEOUT_VAL/256ull;
	}
...
/*****************************************************************************
  Function:
	u8* TCPPutROMString(TCP_SOCKET hTCP, ROM u8* data)
...
/*****************************************************************************
  Function:
	void u16 TCPIsGetReady(TCP_SOCKET hTCP)
...
BOOL TCPGet(TCP_SOCKET hTCP, u8* byte)
{
	...
	{
		MyTCBStub.Flags.bTimer2Enabled = TRUE;
		MyTCBStub.eventTime2 = (u16)TickGetDiv256() + TCP_WINDOW_UPDATE_TIMEOUT_VAL/256ull;
	}
...
/*****************************************************************************
  Function:
	u16 TCPGetArray(TCP_SOCKET hTCP, u8* buffer, u16 len)
...
/*****************************************************************************
  Function:
	u16 TCPFindArrayEx(TCP_SOCKET hTCP, u8* cFindArray, u16 wLen, 
						u16 wStart, u16 wSearchLen, BOOL bTextCompare)
...
u16 TCPFindArrayEx(TCP_SOCKET hTCP, u8* cFindArray, u16 wLen, u16 wStart, u16 wSearchLen, BOOL bTextCompare)
{
	...
	while(1)
	{
		...
		TCPRAMCopy((PTR_BASE)buffer, TCP_PIC_RAM, ptrRead, MyTCBStub.vMemoryMedium, (u16)k);
		ptrRead += k;
		wBytesUntilWrap -= k;
...
/*****************************************************************************
  Function:
	u16 TCPFindROMArrayEx(TCP_SOCKET hTCP, u8* cFindArray, u16 wLen, 
						u16 wStart, u16 wSearchLen, BOOL bTextCompare)
...
/*****************************************************************************
  Function:
	u16 TCPFindEx(TCP_SOCKET hTCP, u8 cFind,
						u16 wStart, u16 wSearchLen, BOOL bTextCompare)
...
void TCPTick(void)
{
	...
	for(hTCP = 0; hTCP < TCP_SOCKET_COUNT; hTCP++)
	{
		...
		if(MyTCBStub.Flags.bTimer2Enabled)
		{
			...
			if((SHORT)(MyTCBStub.eventTime2 - (u16)TickGetDiv256()) <= (SHORT)0)
				vFlags = ACK;
		}
		...
		if(MyTCBStub.Flags.bDelayedACKTimerEnabled)
		{
			...
			if((SHORT)(MyTCBStub.OverlappedTimers.delayedACKTime - (u16)TickGetDiv256()) <= (SHORT)0)
				vFlags = ACK;
		}
		...
		if(MyTCBStub.smState == TCP_CLOSE_WAIT)
		{
			...
			if((SHORT)(MyTCBStub.OverlappedTimers.closeWaitTime - (u16)TickGetDiv256()) <= (SHORT)0)
			{
				vFlags = FIN | ACK;
				MyTCBStub.smState = TCP_LAST_ACK;
			}
		...
		switch(MyTCBStub.smState)
		{
			...
			case TCP_GET_DNS_MODULE:
				if(DNSBeginUsage())
				{
					...
					if(MyTCB.flags.bRemoteHostIsROM)
						DNSResolveROM((ROM u8*)(ROM_PTR_BASE)MyTCB.remote.dwRemoteHost, DNS_TYPE_A);
					else
						DNSResolve((u8*)(PTR_BASE)MyTCB.remote.dwRemoteHost, DNS_TYPE_A);
				}
				break;
			...
			case TCP_GATEWAY_SEND_ARP:
				...
				MyTCBStub.eventTime2 = (u16)TickGetDiv256();
			...
			case TCP_GATEWAY_GET_ARP:
				...
				if(!ARPIsResolved(&MyTCB.remote.niRemoteMACIP.IPAddr, &MyTCB.remote.niRemoteMACIP.MACAddr))
				{
					...
					// never responds
					if((u16)TickGetDiv256() - MyTCBStub.eventTime2 > (u16)MyTCB.retryInterval)
					{
...
//			
		...
		for(w = 0; w < TCP_SYN_QUEUE_MAX_ENTRIES; w++)
		{
			...
			if((u16)TickGetDiv256() - SYNQueue[w].wTimestamp > (u16)(TCP_SYN_QUEUE_TIMEOUT/256ull))
			{
				// Delete this SYN from the SYNQueue and compact the SYNQueue[] array
				TCPRAMCopy((PTR_BASE)&SYNQueue[w], TCP_PIC_RAM, (PTR_BASE)&SYNQueue[w+1], TCP_PIC_RAM, (TCP_SYN_QUEUE_MAX_ENTRIES-1u-w)*sizeof(TCP_SYN_QUEUE));
				SYNQueue[TCP_SYN_QUEUE_MAX_ENTRIES-1].wDestPort = 0u;
...
BOOL TCPProcess(NODE_INFO* remote, IP_ADDR* localIP, u16 len)
{
	...
	checksum1.Val = ~CalcIPChecksum((u8*)&pseudoHeader,
		sizeof(pseudoHeader));
	...
	// Retrieve TCP header.
	IPSetRxBuffer(0);
	MACGetArray((u8*)&TCPHeader, sizeof(TCPHeader));
	SwapTCPHeader(&TCPHeader);


	// Skip over options to retrieve data bytes
	optionsSize = (u8)((TCPHeader.DataOffset.Val << 2)-
		sizeof(TCPHeader));
...
static void SendTCP(u8 vTCPFlags, u8 vSendFlags)
{
	...
	pseudoHeader.Length			= len;
	SwapPseudoHeader(pseudoHeader);
	header.Checksum = ~CalcIPChecksum((u8*)&pseudoHeader, sizeof(pseudoHeader));

	// Write IP header
	MACSetWritePtr(BASE_TX_ADDR + sizeof(ETHER_HEADER));
	IPPutHeader(&MyTCB.remote.niRemoteMACIP, IP_PROT_TCP, len);
	MACPutArray((u8*)&header, sizeof(header));
	if(vTCPFlags & SYN)
		MACPutArray((u8*)&options, sizeof(options));
	...
	MACPutArray((u8*)&wVal, sizeof(u16));
...
/*****************************************************************************
  Function:
	static u16 GetMaxSegSizeOption(void)
...
static void HandleTCPSeg(TCP_HEADER* h, u16 len)
{
	...
	wMissingBytes = (u16)lMissingBytes;
	...
	if(wSegmentLength)
	{
		...
		if(wFreeSpace)
		{
			...
			if((lMissingBytes >= (LONG)0) && (wFreeSpace > (u32)lMissingBytes))
	...
	switch(MyTCBStub.smState)
	{
		...
		case TCP_CLOSING:
			...
			// by this segment.
			wNewWindow = h->Window - ((u16)(MyTCB.MySEQ - localAckNumber));
...
//		return;
	...
	if(len)
	{
		...
		else if((SHORT)wMissingBytes > 0)
		{
			...
			else
			{
				...
				if(wMissingBytes < (u16)MyTCB.sHoleSize)
				{
					if((wMissingBytes + len > (u16)MyTCB.sHoleSize + MyTCB.wFutureDataSize) || (wMissingBytes + len < (u16)MyTCB.sHoleSize))
						MyTCB.wFutureDataSize = len;
					else
						MyTCB.wFutureDataSize = (u16)MyTCB.sHoleSize + MyTCB.wFutureDataSize - wMissingBytes;
					MyTCB.sHoleSize = wMissingBytes;
				}
				else if(wMissingBytes + len > (u16)MyTCB.sHoleSize + MyTCB.wFutureDataSize)
				{
					// Make sure that there isn't a second hole between 
					// our future data and this TCP segment's future data
					if(wMissingBytes <= (u16)MyTCB.sHoleSize + MyTCB.wFutureDataSize)
						MyTCB.wFutureDataSize += wMissingBytes + len - (u16)MyTCB.sHoleSize - MyTCB.wFutureDataSize;
				}
	...
	if(wSegmentLength)
	{
		...
		else
		{
			...
			if(!MyTCBStub.Flags.bDelayedACKTimerEnabled)
			{
				...
				MyTCBStub.OverlappedTimers.delayedACKTime = (u16)TickGetDiv256() + (u16)((TCP_DELAYED_ACK_TIMEOUT)>>8);
			}
		}
	}
	...
	if(localHeaderFlags & FIN)
	{
		...
		// in-order.
		if(MyTCB.RemoteSEQ + 1 == localSeqNumber + (u32)wSegmentLength)
		{
			...
			switch(MyTCBStub.smState)
			{
				...
				case TCP_ESTABLISHED:
					...
					MyTCBStub.OverlappedTimers.closeWaitTime = (u16)TickGetDiv256() + (u16)((TCP_CLOSE_WAIT_TIMEOUT)>>8);
					break;
...
/*****************************************************************************
  Function:
	BOOL TCPAdjustFIFOSize(TCP_SOCKET hTCP, u16 wMinRXSize, 
							u16 wMinTXSize, u8 vFlags)
...
/*****************************************************************************
  Function:
	static void TCPRAMCopy(PTR_BASE ptrDest, u8 vDestType, PTR_BASE ptrSource, 
							u8 vSourceType, u16 wLength)
  ...
  ***************************************************************************/
static void TCPRAMCopy(PTR_BASE ptrDest, u8 vDestType, PTR_BASE ptrSource, u8 vSourceType, u16 wLength)
{
	...
	switch(vSourceType)
	{
		case TCP_PIC_RAM:
			...
			{
				case TCP_PIC_RAM:
					memcpy((void*)(u8*)ptrDest, (void*)(u8*)ptrSource, wLength);
					break;
	
				case TCP_ETH_RAM:
					if(ptrDest!=(PTR_BASE)-1)
						MACSetWritePtr(ptrDest);
					MACPutArray((u8*)ptrSource, wLength);
					break;
	
				#if defined(SPIRAM_CS_TRIS)
				case TCP_SPI_RAM:
					SPIRAMPutArray(ptrDest, (u8*)ptrSource, wLength);
					break;
				#endif
			}
			break;
		...
		case TCP_ETH_RAM:
			switch(vDestType)
			{
				case TCP_PIC_RAM:
					...
					MACGetArray((u8*)ptrDest, wLength);
					break;
		...
		case TCP_SPI_RAM:
			...
			{
				case TCP_PIC_RAM:
					SPIRAMGetArray(ptrSource, (u8*)ptrDest, wLength);
					break;
...
/*****************************************************************************
  Function:
	static void TCPRAMCopyROM(PTR_BASE wDest, u8 wDestType, ROM u8* wSource, 
								u16 wLength)
...
static void TCPRAMCopyROM(PTR_BASE wDest, u8 wDestType, ROM u8* wSource, u16 wLength)
{
	...
	{
		case TCP_PIC_RAM:
			memcpypgm2ram((void*)(u8*)wDest, (ROM void*)wSource, wLength);
			break;
...
/*****************************************************************************
  Function:
	BOOL TCPStartSSLClientEx(TCP_SOCKET hTCP, u8* host, u8 * buffer, u8 suppDataType)
...
void TCPSSLPutRecordHeader(TCP_SOCKET hTCP, u8* hdr, BOOL recDone)
{
	...
	{// This is a new record, so insert the header
		for(i = 0; i < 5u; i++)
		{
			TCPRAMCopy(MyTCBStub.txHead, MyTCBStub.vMemoryMedium, (PTR_BASE)hdr+i, TCP_PIC_RAM, sizeof(u8));
			if(++MyTCBStub.txHead >= MyTCBStub.bufferRxStart)
				MyTCBStub.txHead = MyTCBStub.bufferTxStart;
		}
	}
...
void TCPSSLHandleIncoming(TCP_SOCKET hTCP)
{
	...
	u16 wToMove, wLen, wSSLBytesThatPoofed, wDecryptedBytes;
