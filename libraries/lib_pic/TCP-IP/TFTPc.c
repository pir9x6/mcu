...
u16 _tftpError;                // Variable to preserve error condition causes for later transmission
...
static void _TFTPSendFileName(TFTP_OPCODE command, u8 *fileName);
// Private helper function
static void _TFTPSendAck(WORD_VAL blockNumber);

#if defined(__18CXX)
    // PIC18 ROM variable argument implementation of _TFTPSendFileName
	static void _TFTPSendROMFileName(TFTP_OPCODE opcode, ROM u8 *fileName);
#endif
...
static ROM u8 *vUploadRemoteHost;
static ROM u8 *vUploadFilename;
static TFTP_CHUNK_DESCRIPTOR *uploadChunkDescriptor;
static u16 wUploadChunkOffset;
static CHAR smUpload = TFTP_UPLOAD_COMPLETE;
static TFTP_CHUNK_DESCRIPTOR *uploadChunkDescriptorForRetransmit;
static u16 wUploadChunkOffsetForRetransmit;

/*****************************************************************************
  Function:
	void TFTPUploadRAMFileToHost(ROM u8 *vRemoteHost, 
								 ROM u8 *vFilename, 
								 u8 *vData, 
								 u16 wDataLength)
  ...
  Parameters:
	...
	wDataLength: Number of bytes pointed to by vData.  This will be the final 
		...
		u16 type, the maximum possible file size is 65535 bytes.  For longer 
...
/*****************************************************************************
  Function:
	void TFTPUploadFragmentedRAMFileToHost(ROM u8 *vRemoteHost, 
										   ROM u8 *vFilename, 
...
TFTP_RESULT TFTPIsGetReady(void)
{
    ...
    case SM_TFTP_WAIT_FOR_DATA:
        ...
        if ( opCode.Val == (u16)TFTP_OPCODE_DATA )
        {
        ...
        if ( opCode.Val == (u16)TFTP_OPCODE_ERROR )
        {
...
TFTP_RESULT TFTPIsPutReady(void)
{
    ...
    case SM_TFTP_WAIT_FOR_ACK:
        ...
        if ( opCode.Val == (u16)TFTP_OPCODE_ACK )
        {
        ...
        else if ( opCode.Val == (u16)TFTP_OPCODE_ERROR )
        {
...
static void _TFTPSendFileName(TFTP_OPCODE opcode, u8 *fileName)
{
    u8 c;
