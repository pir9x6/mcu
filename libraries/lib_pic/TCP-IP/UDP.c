...
u16 UDPTxCount;	// Number of bytes written to this UDP segment
u16 UDPRxCount;	// Number of bytes read from this UDP segment
static UDP_SOCKET	LastPutSocket = INVALID_UDP_SOCKET;	// Indicates the last socket to which data was written
static u16 wPutOffset;		// Offset from beginning of payload where data is to be written.
static u16 wGetOffset;		// Offset from beginning of payload from where data is to be read.
                   ...
                   UDP_PORT remotePort)
{
	...
	static u16 NextPort __attribute__((persistent));
...
void UDPFlush(void)
{
	...
	#if defined(UDP_USE_TX_CHECKSUM)
	{
		...
		h.Checksum = ~CalcIPChecksum((u8*)&pseudoHeader, sizeof(pseudoHeader));
	}
	#endif
	...
	#if defined(UDP_USE_TX_CHECKSUM)
	{
		...
		MACPutArray((u8*)&wChecksum, sizeof(wChecksum));
	}
	#endif
...
BOOL UDPProcess(NODE_INFO *remoteNode, IP_ADDR *localIP, u16 len)
{
	...
	if(h.Checksum)
	{
	    ...
	    checksums.w[0] = ~CalcIPChecksum((u8*)&pseudoHeader,
	                                    sizeof(pseudoHeader));
