...
static u16 wTXWatchdog;	// Time of last transmission (high resolution); used for determining when TX hardware may need software intervention
#if defined(ETH_RX_POLARITY_SWAP_TRIS)
	static u16 wRXPolarityTimer;	// Time of last transmission (long duration); used for determining when a RX polarity swap may be needed
#endif
...
BOOL MACIsTxReady(void)
{
	...
	if((u16)TickGet() - wTXWatchdog >= (3ull*TICK_SECOND/1000ull))
	{
		ECON1bits.TXRTS = 0;
		MACFlush();
	}
...
BOOL MACGetHeader(MAC_ADDR *remote, u8* type)
{
	...
	if(EPKTCNT == 0u)
	{
		...
		#if defined(ETH_RX_POLARITY_SWAP_TRIS)
		{
			...
			if((u16)TickGetDiv256() - wRXPolarityTimer > (u16)(TICK_SECOND*3/7/256))
			{
				...
				wRXPolarityTimer = (u16)TickGetDiv256();
	...
	MACGetArray((u8*)&header, sizeof(header));
...
void MACPutHeader(MAC_ADDR *remote, u8 type, u16 dataLen)
{
	...
	// address
    MACPutArray((u8*)remote, sizeof(*remote));

	// Write our MAC address in the Ethernet source field
	MACPutArray((u8*)&AppConfig.MyMACAddr, sizeof(AppConfig.MyMACAddr));
...
/*
void MACMemCopyAsync(u16 destAddr, u16 sourceAddr, u16 len)
{
	WORD_VAL ReadSave, WriteSave;
	BOOL UpdateWritePointer = FALSE;
	BOOL UpdateReadPointer = FALSE;
...
void WritePHYReg(u8 Register, u16 Data)
{
	...
	#if defined(HI_TECH_C)
		errataTempL = ((u8*)&Data)[0];
		errataTempH = ((u8*)&Data)[1];
