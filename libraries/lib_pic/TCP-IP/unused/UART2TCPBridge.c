...
static u8 vUARTRXFIFO[65];
static u8 vUARTTXFIFO[17];
static volatile u8 *RXHeadPtr = vUARTRXFIFO, *RXTailPtr = vUARTRXFIFO;
static volatile u8 *TXHeadPtr = vUARTTXFIFO, *TXTailPtr = vUARTTXFIFO;
...
void UART2TCPBridgeTask(void)
{
	...
	u16 wMaxPut, wMaxGet, w;
	u8 *RXHeadPtrShadow, *RXTailPtrShadow;
	u8 *TXHeadPtrShadow, *TXTailPtrShadow;
	...
	switch(BridgeState)
	{
		case SM_HOME:
				...
				MySocket = TCPOpen((u32)USE_REMOTE_TCP_SERVER, TCP_OPEN_ROM_HOST, UART2TCPBRIDGE_PORT, TCP_PURPOSE_UART_2_TCP_BRIDGE);
			#else
		...
		case SM_SOCKET_OBTAINED:
			...
			RXTailPtrShadow = (u8*)RXTailPtr;
			TXHeadPtrShadow = (u8*)TXHeadPtr;
			#if defined(__18CXX)
			PIE1bits.RCIE = 0;
			PIE1bits.TXIE = 0;
			#else
			...
			IEC1bits.U2TXIE = 0;
			#endif
			RXHeadPtrShadow = (u8*)RXHeadPtr;
			TXTailPtrShadow = (u8*)TXTailPtr;
			#if defined(__18CXX)
			PIE1bits.RCIE = 1;
			...
			IEC1bits.U2TXIE = 0;
			#endif
			RXTailPtr = (volatile u8*)RXTailPtrShadow;
			TXHeadPtr = (volatile u8*)TXHeadPtrShadow;
			#if defined(__18CXX)
			PIE1bits.RCIE = 1;
			if(TXHeadPtrShadow != TXTailPtrShadow)
				PIE1bits.TXIE = 1;
			...
			if((RXHeadPtr != vUARTRXFIFO + sizeof(vUARTRXFIFO)) || (RXTailPtr != vUARTRXFIFO))
			{
				*RXHeadPtr++ = i;
				if(RXHeadPtr >= vUARTRXFIFO + sizeof(vUARTRXFIFO))
					RXHeadPtr = (volatile u8*)vUARTRXFIFO;
			}
		}
	}
	...
	if(PIR1bits.TXIF)
	{
		if(TXHeadPtr != TXTailPtr)
		{
			...
			if(TXTailPtr >= vUARTTXFIFO + sizeof(vUARTTXFIFO))
				TXTailPtr = (volatile u8*)vUARTTXFIFO;
		}
