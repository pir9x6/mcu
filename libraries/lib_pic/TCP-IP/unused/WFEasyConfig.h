...
typedef struct {
    u8            isWifiNeedToConfigure;
    u8            isWifiDoneConfigure;
    u8            ssid[33];               // 32 for ssid and one for term character
    tWFNetworkType  type;                   // Net type infrastructure vs adhoc
    u8            prevSSID[33];           // previous SSID we were connected to
    u8            prevWLAN;               // previous WLAN type
    
    u8            security;               // security type
    u8            defaultWepKey;          // WEP index value
    u8            key[65];                // 64-byte key plus term character
