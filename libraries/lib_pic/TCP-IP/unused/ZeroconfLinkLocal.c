...
const u8 g_zcll_dhcp_client = 1;
#else
const u8 g_zcll_dhcp_client = 0;
#endif
...
#endif
static u8 probe_count = 0;
static u8 conflict_count = 0; 
static u8 announce_count = 0;
...
void ZeroconfARPPktNotify (u32 SenderIPAddr, u32 TargetIPAddr, 
                           MAC_ADDR* SenderMACAddr, 
                           MAC_ADDR* TargetMACAddr, u8 op_req);
...
/***************************************************************
  Function:
	void ARPAction(IP_ADDR SrcIPAddr,IP_ADDR DestIPAddr ,u8 op_req,ARP_STATE ARPAction)
...
void ARPAction(u32 SrcIPAddr,u32 DestIPAddr ,u8 op_req,ARP_STATE ARPAction)
{
	BOOL rc;
...
/***************************************************************
  Function:
	ARP_PKT_TYPE FindARPPktType (u32 SrcIPAddr, u32 DestIPAddr, 
                                 u8 op_req)
  ...
  ***************************************************************/
ARP_PKT_TYPE FindARPPktType (u32 SrcIPAddr, u32 DestIPAddr,u8 op_req)
{
	if(op_req == ARP_REQ)
	{
		if(SrcIPAddr == 0x0)
	...
	case SM_ADDR_PROBE:
			...
			(!bDefaultIPTried))
		{
			...
			*/
			...
			temp_IP_addr.Val = swapl((u32) temp_IP_addr.Val);
		}
