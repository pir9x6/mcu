...
typedef struct _mDNSResourceRecord
{
	...
	u8 *rdata;		// for PTR, SRV and TXT records. 
    ...
    u8 valid; /* indicates whether rr is valid */
...
typedef struct _mDNSProcessCtx_common
{
	...
	u8 time_recorded; // Flag to indicate event_time is loaded
	TICK random_delay;
...
typedef struct _mDNSProcessCtx_host
{
	...
	u8 szUserChosenHostName[MAX_HOST_NAME_SIZE];	// user chosen host name
	u8 szHostName[MAX_HOST_NAME_SIZE];					// mDNS chosen Host-Name
...
typedef struct _mDNSProcessCtx_sd
{
	...
	// info specific to SD
    u8 srv_name[MAX_SRV_NAME_SIZE];
    u8 srv_type[MAX_SRV_TYPE_SIZE];
    u8 sd_qualified_name[MAX_RR_NAME_SIZE];
    u8 used; /* Spinlock to protect Race-cond. */
    
    u8 sd_auto_rename: 1,        /* Flag to show auto-Rename is enabled */
    ...
    u16 sd_port; /* Port number in Local-sys where Service is being offered */
    u8 sd_txt_rec[MAX_TXT_DATA_SIZE];
    u8 sd_txt_rec_len;
...
const u8 *CONST_STR_local = (u8 *) "local";
...
MDNS_STATIC u16 mDNSDeCompress(u16 wPos, u8 *pcString, BOOL bFollowPtr, u8 cElement, u8 cDepth);
static u16 g_mDNS_offset;
...
//#define MDNS_USE_LOCAL_ITOA		y	//local version returns extra info. To be fixed.
	...
	#define ITOA_LOCAL(x,y)			uitoa((u16)x,y)
#endif

#ifdef MDNS_USE_LOCAL_STRLEN
MDNS_STATIC u8 strlen_local(u8 *str);
#endif
...
/***************************************************************
  Function:
	void DisplayHostName(u8 *HostName)
  ...
  Returns:
  	None
  ***************************************************************/
MDNS_STATIC void DisplayHostName(u8 *HostName)
{
    u8 SingleChar;
	u8 i;
	u8 len;
...
/***************************************************************
  Function:
	static u8 strlen_local(u8 *string)
...
/***************************************************************
  Function:
	static u8 strcmp_local(u8 *string_1, u8 *string_2)
  ...
  **************************************************************/
MDNS_STATIC u8 strcmp_local(u8 *str_1, u8 *str_2)
{
...
/***************************************************************
  Function:
	static u8 strcmp_local_ignore_case(u8 *string_1, u8 *string_2)
  ...
  Returns:
    ...
    Non-Zero: If both strings are not equal or on error case  	
  **************************************************************/
MDNS_STATIC u8 strcmp_local_ignore_case(u8 *str_1, u8 *str_2)
{
    if(str_1 == NULL || str_2 == NULL)
    {
...
/********************************************************************
  Function:
	static void strcpy_local(u8 *dst_string, u8 *src_string)
  ...
  Returns:
    None  	
  **************************************************************/
MDNS_STATIC void strcpy_local(u8 *dst, u8 *src)
{
    while((*dst++ = *src++));
}
...
/***************************************************************
  Function:
	static void strrev_local(u8 *string)
  ...
  Returns:
  	None
  ***************************************************************/
MDNS_STATIC void strrev_local(u8 *str)
{
	u8 *first, *last;
	u8 ch;
...
/***************************************************************
  Function:
	static u8 itoa_local(u8 num, u8 *string)
  ...
  **************************************************************/
MDNS_STATIC u8 itoa_local(u8 num, u8 *str)
{
	u8 *tmp = str;
	u8 cnt = 0;
...
/***************************************************************
  Function:
	static void mDNSRename(u8 *str, u8 max_len)
...
MDNS_STATIC void mDNSRename(u8 *strLabel, u8 nLabelId, u8 *strBase, u8 *strTarget, u8 nMaxLen)
{
	u8 n = nLabelId;
#define mDNSRename_ID_LEN 6
	u8 str_n[mDNSRename_ID_LEN]; //enough for "-255." + '\0'.
	u8 i = mDNSRename_ID_LEN - 1 ;
...
/***************************************************************
  Function:
	static void mDNSPutString(u8* String)
...
MDNS_STATIC void mDNSPutString(u8* string)
{
	u8 *right_ptr,*label_ptr;
    u8 label[MAX_LABEL_SIZE];
	u8 i;
	u8 len;
...
/***************************************************************
  Function:
	static BOOL mDNSSendQuery(u8* name, u8 record_type)
  ...
  **************************************************************/
//MDNS_STATIC BOOL mDNSSendQuery(u8* name, u8 record_type, u8 cFlush, u8 probe_type)
...
MDNS_STATIC BOOL mDNSProbe(mDNSProcessCtx_common *pCtx)
{
	...
	*/
	...
	mDNS_header.nQuestions.Val = swaps(((u16)1u));					// 1 entry in the question section
	mDNS_header.nAnswers.Val = 0;									// 0 entry in the answer section
	mDNS_header.nAuthoritativeRecords.Val = swaps(((u16)1u));		// 1 entry in name server section
	mDNS_header.nAdditionalRecords.Val = 0;							// 0 entry in additional records section

	// Put out the mDNS message header
	UDPPutArray((u8 *) &mDNS_header, sizeof(MDNS_MSG_HEADER));
...
/***************************************************************
  Function:
	static BOOL mDNSSendRR(struct mDNSResourceRecord *record, 
                   u8 record_type, u32 ttl_val,u16 query_id)
...
/***************************************************************
  Function:
	void mDNSSDFormatServiceInstance(u8 *string)
...
{
	u8 *temp;
	u8 output[MAX_LABEL_SIZE];
	u8 i;
	u8 *right_ptr,*str_token;
	u8 len;
...
void mDNSSDFillResRecords(mDNSProcessCtx_sd *sd)
{
    ...
    *qualified_name++ = '.';
	...
	rr_list->name = (u8 *) (sd->srv_type);

    /* Res Record Name is 
     * Service_Instance_name._srv-type._proto.domain */
	rr_list->rdata = (u8 *) (sd->sd_qualified_name);
    ...
    *qualified_name++ = '.';
    ...
    /* Fill-up SRV Record */
	rr_list->name = (u8 *) (sd->sd_qualified_name);
    ...
    /* Res Record Name is 
     * Service_Instance_name._srv-type._proto.domain */
	rr_list->rdata = (u8 *) gHostCtx.szHostName;
	...
	rr_list->rdata = (u8 *) (sd->sd_txt_rec);
				  ...
				  const u8	*txt_record)
{
    ...
    if( sd->used)
    {
        ...
        if(txt_record != NULL)
        {
			STRCPY_LOCAL(sd->sd_txt_rec, (u8 *) txt_record);
					...
					void *context)
{
    ...
    STRCPY_LOCAL(gSDCtx.srv_name, (u8 *)srv_name);
    
    STRCPY_LOCAL(gSDCtx.srv_type, (u8 *)srv_type);
    gSDCtx.sd_call_back = call_back;
    gSDCtx.sd_context = context;

	STRCPY_LOCAL(gSDCtx.sd_txt_rec, (u8 *) txt_record);
...
MDNS_STATIC u16 mDNSFetch(u16 wOffset, u16 wLen, u8 *pcString)
{
	u16 rc;
...
/***************************************************************
  Function:
	static u16 mDNSDeCompress(u16 wPos, 
                               u8 *pcString, 
                               BOOL bFollowPtr, 
                               u8 cElement, 
                               u8 cDepth)
...
MDNS_STATIC u16 mDNSDeCompress(u16 wPos, u8 *pcString, BOOL bFollowPtr, u8 cElement, u8 cDepth)
{
	u16 rr_len = 0; // As is in the packet. Could be in compressed format.
	u16 startOffset, endOffset, currOffset;
	u8 i, tmp;
	u16 offset_in_ptr;
	u16 len;
	u8 substr_len;
					  ...
					  u16				idxRR)
{
	...
	for (i = 0; i < MAX_RR_NUM; i++)
	{
			...
			)
		{
			...
			&&
			!strcmp_local_ignore_case(name,(u8 *) "_services._dns-sd._udp.local")
			&&
...
MDNS_STATIC void mDNSResponder(void)
{
	...
	switch(mDNS_responder_state)
	{
		...
		case MDNS_RESPONDER_LISTEN:
			...
			len = mDNSFetch(0, sizeof(mDNS_header), (u8 *) &mDNS_header);
...
/***************************************************************
  Function:
	static void mDNSProbe(u8 *name, MDNS_QTYPE q_type)
...
/***************************************************************
  Function:
	static void mDNSAnnounce(u8 *name, MDNS_QTYPE q_type)
...
MDNSD_ERR_CODE mDNSHostRegister(const char *host_name)
{
	if (host_name != NULL)
	{
		STRCPY_LOCAL(gHostCtx.szUserChosenHostName, (u8 *) host_name);
	}
	else 
	{
#ifdef MY_DEFAULT_HOST_NAME
		STRCPY_LOCAL(gHostCtx.szUserChosenHostName, (u8 *) MY_DEFAULT_HOST_NAME);
#else
		STRCPY_LOCAL(gHostCtx.szUserChosenHostName, (u8 *) "MyHost");
#endif
	}

	STRCPY_LOCAL(gHostCtx.szHostName, gHostCtx.szUserChosenHostName);
	STRCPY_LOCAL(gHostCtx.szHostName+STRLEN_LOCAL(gHostCtx.szHostName), (u8 *) ".");
	STRCPY_LOCAL(gHostCtx.szHostName+STRLEN_LOCAL(gHostCtx.szHostName), (u8 *) CONST_STR_local);
