...
result_t ds1307_init (I2C_BUS i2c_bus_id, u8 adr_chip)
{
    ...
    if (i2c_write(i2c_bus_id, 0x0E) != SUCCESS)
        return ERROR;
...
result_t ds1307_get_time (I2C_BUS i2c_bus_id, u8 adr_chip, date_time_t *t)
{
    ...
    if (i2c_read_reg(i2c_bus_id, adr_chip, 0x00, &tmp) != SUCCESS)
        return ERROR;
    bcd_2_bin(tmp, &t->sec);

    if (i2c_read_reg(i2c_bus_id, adr_chip, 0x01, &tmp) != SUCCESS)
        return ERROR;
    bcd_2_bin(tmp, &t->min);

    if (i2c_read_reg(i2c_bus_id, adr_chip, 0x02, &tmp) != SUCCESS)
        return ERROR;
    bcd_2_bin(tmp, &t->hrs);

    if (i2c_read_reg(i2c_bus_id, adr_chip, 0x03, &tmp) != SUCCESS)
        return ERROR;
    bcd_2_bin(tmp, &t->dow);

    if (i2c_read_reg(i2c_bus_id, adr_chip, 0x04, &tmp) != SUCCESS)
        return ERROR;
    bcd_2_bin(tmp, &t->day);

    if (i2c_read_reg(i2c_bus_id, adr_chip, 0x05, &tmp) != SUCCESS)
        return ERROR;
    bcd_2_bin(tmp, &t->mth);

    if (i2c_read_reg(i2c_bus_id, adr_chip, 0x06, &tmp) != SUCCESS)
        return ERROR;
    bcd_2_bin(tmp, &t->yrs);
